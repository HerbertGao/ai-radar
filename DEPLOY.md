...
4. **镜像 digest 钉死**：部署前把 compose 里 `cloudflare/cloudflared:latest` 换成 `cloudflare/cloudflared:<版本>@sha256:<digest>`（网络面守护进程，防 CVE 陈旧）。

5. **起服务**：`docker compose --profile app up -d cloudflared`（随现有 web/worker 栈起，依赖 `web` healthy）。
