...
实时告警（`realtime-alerts`）默认关闭（`ALERT_SCAN_ENABLED=false`）。启用会让 `published_at` 近 N 天内、达阈值（`importance_score >= ALERT_IMPORTANCE_THRESHOLD`）、从未告警的**存量**事件在启用瞬间成为候选 → 批量推送旧消息（刷屏，违反 `policy-push-timeliness`）。故基线与启用**必须分离**，按此次序部署：

1. **合并代码，默认仍关**：代码合并后 `ALERT_SCAN_ENABLED` 保持 `false`，worker 完全跳过告警链。
2. **先设发布时间基线水位**：把 `ALERT_MIN_PUBLISHED_AT` 设为**启用时刻**的 ISO 时刻（如 `node -e "console.log(new Date().toISOString())"` 的输出），只告警该基线之后发布的新闻，启用前发布的存量一律排除。
   - 若确不需要基线（接受旧消息刷屏风险），显式设为空串 `ALERT_MIN_PUBLISHED_AT=` 明示 opt-out。
3. **再改生产 cron 覆盖值**：`ALERT_SCAN_CRON=9-59/20 * * * *`（分钟展开集 `{9,29,49}`）。**这一步 MUST NOT 省略**——机械断言只读代码里的默认值（现默认 `4-59/15`），只改默认值而不改生产覆盖值 = CI 全绿而生产照撞整点。
   - **不要用 `3-59/20`**：其 `{3}` 与日报 `DAILY_DIGEST_CRON` 的 `{3}` 在 **08:03 撞同一分钟**。
   - 选值原则：分钟展开集既避开 `{0,30}`，也避开其余飞书 cron 的分钟集（`{3}` 日报 / `{7}` 周报 / `{53}` MR 价格复核）。`{9,29,49}` 与三者均不相交。
4. **最后再置开关启用**：`ALERT_SCAN_ENABLED=true`，`up -d` 使 worker 注册告警链。

**代码已强制该次序**（非仅靠本文档）：`ALERT_SCAN_ENABLED=true` 但 `ALERT_MIN_PUBLISHED_AT` **未设置**（既非 ISO 亦非显式空串）时，env 校验在启动阶段 **fail-fast**、worker 起不来——防「启用却忘设基线 → 存量 P0 刷屏」。非法 ISO 亦启动即报错。回滚 = 置 `ALERT_SCAN_ENABLED=false` 重启。
...
## 对话 RAG `/advisor` 公网暴露与多层安全（add-conversational-rag A3，design D10/D11）

`/advisor` 是**公开 LLM 端点**（多轮 chat + 每轮 1–2 次 OpenRouter 调用），随现有 `web` 进程与 Model Radar **同一 tunnel**。**不以 CF Access 为单点**——承重层是 **in-app CF Access JWT 校验**（即便边缘 CF Access 误配 / 请求直连绕过 origin 亦被 in-app 拦）。

1. **ingress 显式放行 `/advisor`**（`cloudflared/config.yml`）：现 tunnel 仅放行 `/model-radar` + assets，其余边缘 404。加一条 hostname/path 规则放行 `/advisor`（含 `POST`）。**不放行** `/health` 等其余路径。

2. **CF Access 对 `/advisor` 必填**（**非**「可选、用户拍板」——它是公开 LLM 端点）：为该 path 建 CF Access 应用（邮箱 OTP/SSO）。取该应用的 **AUD tag** 填 `CF_ACCESS_AUD`，team 域（如 `myteam.cloudflareaccess.com`）填 `CF_ACCESS_TEAM_DOMAIN`（应用据此派生 `iss=https://<域>`、`jwks_uri=https://<域>/cdn-cgi/access/certs`）。**两项任一为空 → `/advisor` fail-closed 拒绝服务（503），绝不半开放**。

3. **in-app JWT（承重层，代码强制、不预设 tunnel-only）**：`src/rag/jwt-middleware.ts` 用 Hono 内置 `hono/jwk`（**不手写 RS256**）读 `CF_Authorization` cookie 的裸 JWT，校验 `aud`+`iss`+`exp`、pin `alg:['RS256']`、JWKS 拉取失败 **fail-closed 拒绝**。只挂 `/advisor` + `/advisor/*`、**不挂**公开只读的 `/model-radar`/`/health`；跑在每日成本闸与任何 LLM 调用**之前**。

4. **CF WAF per-IP 限流** `/advisor`：镜像 `/model-radar` 的 **60/IP/min · block · 10min**，规则**须限流触发 LLM 的提交 `POST`**（非仅 GET 页面加载），防单个已鉴权会话秒级刷穿每日上限。

5. **「只经 tunnel」靠既有拓扑、不绑 loopback**：沿用既有 docker 网络分段；把「撤宿主 `:3000` 映射」作为显式部署步骤（同上文 Model Radar 第 6 步，现默认保留）。**MUST NOT 绑 `127.0.0.1`**——全 app 单 `serve({port})`，绑 loopback 会锁死 Model Radar 且打断 cloudflared sidecar 经 docker DNS(`web:3000`) 回源。即便宿主映射未撤、请求直连 origin，in-app JWT 仍拦住 `/advisor`。

6. **最小成本/输入下限（env，design D11）**：`RAG_MAX_QUERY_CHARS`（query 超长拒绝、不触发 LLM，默认 4000）；`RAG_DAILY_LLM_CALL_CAP`（全局每日 LLM 调用上限，Redis `INCR rag:llmcalls:<UTC-date>` + 每次幂等 `EXPIRE … NX`——越限**或 Redis 不可用**均 fail-closed 停止作答，绝不 fail-open，默认 500）。**要求 Redis ≥ 7.0**（`EXPIRE … NX` 标志自 7.0 起）：compose 已用 `redis:7-alpine`；若换更旧 Redis，每次成本闸检查都会抛错 → `/advisor` 恒 429 自我 DoS。该上限是**按提交 POST 计的请求地板**（每 POST 约 1–2 次 LLM 调用；价格前置闸/无据 POST 也占一格），非精确 per-call 计数——刻意粗粒度（design D11）。

7. **MCP `search_kb` 证据出口所需凭据（转交自组 A）**：若同时启用 MCP `search_kb`，须 **`LLM_API_KEY` + `LLM_BASE_URL` + `EMBEDDING_MODEL` 三项齐**（mcp 侧无 default、缺任一 fail-closed，其余工具 + server 照常）——DEPLOY 明确三项、非仅 `LLM_API_KEY`。

8. **部署验证清单**：
   - 公网 `GET /advisor`（无 CF 会话）→ 经 CF Access 重定向登录；登录后带 `CF_Authorization` cookie → 200 chat 页。
   - **直连 origin**（绕过边缘、缺 `Cf-Access-Jwt-Assertion`/`CF_Authorization`）→ in-app JWT **401**（CF Access 误配/绕过不致 `/advisor` 全开）。
   - `CF_ACCESS_AUD`/`CF_ACCESS_TEAM_DOMAIN` 未配 → `/advisor` 恒 **503**（fail-closed）；`/model-radar`/`/health` 不受影响。
   - 高频 `POST /advisor` 洪泛 → 触发 WAF 限流；达 `RAG_DAILY_LLM_CALL_CAP` → 429「已达当日上限」、不再烧 OpenRouter；`query` 超 `RAG_MAX_QUERY_CHARS` → 400 拒绝。
   - Redis 不可用时 `POST /advisor` → 429 fail-closed（**不** fail-open 作答）。

9. **回滚**：从 `config.yml` 撤 `/advisor` ingress 规则 + 停 CF Access 应用 → 公网即不可达（应用零影响，`rag_conversations` 与其余车道不动）。
