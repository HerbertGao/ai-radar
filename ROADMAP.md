...
> **5e v2（已定案 `add-model-radar-recommender-rag-explanation`）**：模板解释层换为 RAG 证据（复用 `searchKbCore` env-clean 检索核心 + `mr_price_history` 变更流 + 待复核标）+ 可选 LLM 证据叙述段，召回/候选 schema/verdict 不动（`ExplanationInput→Promise` 接口留缝已兑现）。**默认 `MR_RECOMMEND_EXPLAIN=template`（部署即惰性），生产开 `llm` 是独立运营决策**；结论与数字产出权恒在程序侧（模板段权威 + 三道机械守卫），任何失败逐字节回落 v1。
