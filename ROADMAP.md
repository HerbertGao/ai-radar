...
| **P5** ✅ | **Model Radar — 编程订阅比价 + 选型（编程垂类，5a–5e + Web 前端两段式 A/B 全部落地并归档）** | 5–7 周 | —（P0–P4 提前完成，按日历重定基线） | 06-25 ~ 08-09 | 见下「[P5 Model Radar 步骤拆解](#p5-model-radar-步骤拆解)」5a–5e：数据模型+provenance / 录入+保鲜回路 / 桶2数据+比价检索API / Web 比价页 / 垂类选型推荐器；+ Web 前端精修（A 设计系统 / B 答案优先 IA） | P2（产品·事件流）+ P4（MCP） | **退出标准达成**：比价页 10s 内答「谁含 GLM-4.6 / 谁支持 Claude Code / 同档谁最划算 / 谁最近变了」（5d-B）；推荐器答「重度 Claude Code + GLM-4.6 最便宜可用」+ 撞窗 + 月成本 + 依据（5e）；每条价/兼容/额度带 `source_url`+`last_checked`+`source_confidence`；陈旧项可被 ai-radar 变更流标「待复核」；**答案优先 Web**（P5 B）把推荐器搬上 `/model-radar` 作 hero、比价表降为可展开证据抽屉 |
...
**关键路径**：P0 → P1 → P2 → P3 → **P5（Model Radar）✅**。P0–P5 已上线（含情报流 / 双通道 / 语义去重 / 知识库 / MCP / **Model Radar 比价页 + 选型推荐器 + 答案优先 Web 页**）；**P5 Model Radar 全部步骤（5a–5e）+ Web 前端两段式（A 设计系统 / B 答案优先）落地完毕**，下一里程碑 = **P6 泛化选型顾问**（Model Radar 超集，把验证过的「规则召回 → RAG → LLM 解释」从编程垂类泛化到「做某事用哪个 AI 工具」）。

> **进度（截至 2026-07-21）**：**P0–P4 关键路径全部落地**，外加 roadmap 外的「AI 博主经验挖掘」链（归档 `add-ai-blogger-experience-mining`）。**P5 Model Radar：5a / 5b / 5c / 5d-A / 5d-B / 5d-C / 5e 全部落地并归档**——5a 数据模型（`add-model-radar-data-model`）、5b 录入+保鲜回路+三档抓取变更检测（`add-model-radar-ingestion-freshness`，PR #45）、5c 比价/检索 API + 只读快照 + 桶2数据红线（`add-model-radar-compare-api`，PR #46）、5d-A 快照跨进程失效 Redis pub/sub + 服务进程周期 rebuild（`add-model-radar-snapshot-cross-process-invalidation`，PR #47）、5d-B Hono JSX SSR 比价 Web 页 + per-fact age 徽标（`add-model-radar-compare-web-page`，PR #48）、5d-C 桶2真价人工策展（`add-model-radar-bucket2-price-curation`，PR #49：经既有授权改价入口录 6 家 CNY 官方真月价，比价页转出真实最划算 = 讯飞 ¥19）、**5e 垂类选型推荐器 v1（规则 + 模板）+ MCP 单工具**（`add-model-radar-recommender`，PR #50：vetted money-path 召回 + `fitsWindow` 撞窗纯数值原语 + 四态有序 verdict + 模板解释层 + env-clean MCP `recommend_coding_subscription`；退出用例「重度 Claude Code + GLM-4.6」→ 首选 GLM Lite ¥49）。**Web 前端两段式精修（P5 A/B）全部落地并归档**——**变更 A** 比价页产品仪表面板设计系统（`refresh-model-radar-compare-web-design-system`，PR #59：分层 surface/token + 暗色-ready + 自托管 Hanken Grotesk webfont + CSS View Transitions + CSS 状态记号弃 emoji）、**变更 B** 答案优先 IA（`add-model-radar-answer-first-web`，PR #60：把 5e 推荐器搬上 `/model-radar` 作 hero——深色定论答案卡 + 结构化推荐说明表 + 备选账本 + 「描述你的配置」表单 + 比价表降为证据抽屉；零客户端 JS、WCAG 2.2 AA、thin-data 诚实；money-path / 引擎 / DTO 不改；经 3 轮 review-loop 修 6 项 + 4 项回归断言）。主规范现有 `model-radar-catalog`(5a) / `model-radar-ingestion`(5b+5c 红线) / `model-radar-compare-api`(5c+5d-A) / `model-radar-compare-web`(5d-B，A 精修设计系统 + B 加「表可作证据抽屉嵌入」) / `model-radar-recommender`(5e) / `model-radar-answer-first-web`(B) 六谱。**P5 Model Radar 全部步骤（5a–5e）+ Web 前端两段式（A/B）落地完毕**（browser egress 生产启用 + 自动抓取已**拆出延后**为独立步骤、不在 P5 关键路径，见步骤表）；下一步 = **P6 泛化选型顾问**（Model Radar 超集，把「规则召回 → RAG → LLM 解释」从编程垂类泛化）或持续运营。已了结：**5e v2**（RAG 证据 + LLM 解释，仅换可插拔解释层、召回不动——`add-model-radar-recommender-rag-explanation`，PR #90：证据装配 + 三守卫 + canonical，默认 template、生产开 llm 独立运营决策，归档待合并）；P3 语义阈值真实数据复校（**2026-07-17 完成**：生产 25 天护栏后语料复算——`SEMANTIC_DEDUP_LLM=0.75` 转正、误并根源系冷启动回填非阈值、新源回填期临时用严值 0.82，详见当次复校记录）。**browser 档 URL-drift agent 已交付**（`add-model-radar-browser-url-drift-agent`，PR #93，07-21）：browser 档源 URL 失效时 agent 提候选新 URL、经 Telegram HITL 批准后由 authorized setter `setSourceUrl` 改 `mr_source.source_url`（C4 ruled fact 调和——同 allowlist 域内 drift 才参与、跨域走 PR 流程；默认 `MR_URL_DRIFT_ENABLED='false'` 门控关）；经 5 轮代码 review-loop 修 1 blocker + 4 major + 残余登记。主规范新增第 7 谱 `model-radar-url-drift-agent`。
...
| **Web-A** ✅ | 比价页**产品仪表面板设计系统**（5d-B 之上精修）：分层 surface/token + 暗色-ready 调色 + 自托管 Hanken Grotesk webfont（放行 CSP `font-src 'self'`）+ CSS View Transitions（reduced-motion 置零）+ CSS 绘制状态记号（弃 emoji）；控件 a11y 驯化（`--border-control` ≥3:1、outline 焦点、目标 ≥24px） | ~1 周 | **已落地归档**（`refresh-model-radar-compare-web-design-system`，PR #59）：比价页换分层产品界面语言、逐层对比度 ≥4.5:1 实测、零客户端 JS 不变 |
| **Web-B** ✅ | **答案优先 IA**：把 5e 推荐器搬上 `/model-radar` 作 hero（翻转「一张比价表」为「先给答案 + 表作证据」）——深色定论答案卡（primary）+ 结构化「推荐说明表」(从 candidates 零判定渲染) + 备选账本 + 「描述你的配置」表单 + 变更 A 比价表降为可展开证据抽屉；呈现层零判定（全由 `recommend()` 输出映射）、setup 校验边界防 fail-open、money-path/引擎/DTO 不改 | ~1.5 周 | **已落地归档**（`add-model-radar-answer-first-web`，PR #60；新建主规范 `model-radar-answer-first-web` +8 需求、`model-radar-compare-web` ~2 需求）：`/model-radar` 先给答案卡再给证据；经 3 轮 review-loop 修 6 项（含畸形预算→400、表单回显生效档、深色卡焦点环 WCAG 2.4.11）+ 4 项回归断言 |
...
> **5e v2（已定案 `add-model-radar-recommender-rag-explanation`）**：模板解释层换为 RAG 证据（复用 `searchKbCore` env-clean 检索核心 + `mr_price_history` 变更流 + 待复核标）+ 可选 LLM 证据叙述段，召回/候选 schema/verdict 不动（`ExplanationInput→Promise` 接口留缝已兑现）。**默认 `MR_RECOMMEND_EXPLAIN=template`（部署即惰性），生产开 `llm` 是独立运营决策**；结论与数字产出权恒在程序侧（模板段权威 + 三道机械守卫），任何失败逐字节回落 v1。**公开页 `/model-radar` 开 `llm` 的成本边界前置已实现**（`add-model-radar-explain-public-cost-bound`）：整条解释缓存（快照版本+setup 哈希）+ 独立 namespace 日 LLM 上限（键 `mr:llmcalls:`、与 advisor 预算隔离、fail-open 回落模板、页恒 200；`MR_EXPLAIN_DAILY_LLM_CAP` 默认 200，计逻辑作答、日真调 ≈ 2× cap），经 `buildExplainer` opt-in 回调注入、MCP 不注入保 env-clean。**第二条前置「装配 deadline 真取消」亦已实现**（`add-model-radar-assembly-deadline-cancel`）：证据装配超时经统一 `deadlineAt` 真取消底层调用——embed 走 `AbortSignal`（`embedTexts`/`embedTextsClean` 两条实现、abort 不重试）、DB 查询走单连事务内 `set_config('statement_timeout', 剩余预算, true)`（`remainingMs` 回调内算防满池获取等待跨 deadline、异常自动 ROLLBACK 释放连接），`signal`/`deadlineAtMs` 全 opt-in、缺省逐字节不变、其它 embed/searchKbCore 消费者零影响。
> **RAG 检索路径选型（建 5e v2 证据层 / P6 「规则召回 → RAG」时读）**：读路径**已建**——`add-conversational-rag` 落了 `src/kb/retrieval-core.ts`（env-clean pgvector 余弦检索 + tombstone 反连接 + top-k 归一化），5e v2 证据层即复用它（定序第①步 baseline 已就位）。评估过 [SAG](https://github.com/Zleap-AI/SAG)（pgvector 之上的轻量实体-事件索引 `chunk→event`/`chunk→entities`，用 SQL 做多跳召回；栈与本仓 **1:1** = TS + PG + pgvector + MCP SDK，MIT）。**定序：① 先上最朴素 pgvector 余弦检索精选 KB 作 baseline → ② 量哪些查询败在跨文档实体串联（多跳）→ ③ 真有多跳瓶颈再引 SAG**。理由：**(a)** 同栈 = 延后零锁定、无需趁早接（这是可放心推迟的理由，不是趁早接的理由）；**(b)** SAG 优势（多跳*散文*召回）碰不到 Model Radar 钱路——价 / 额度 / 兼容是 `mr_*` 结构化精确事实、走 SQL 非检索，那里的「多跳」本就是表 JOIN；**(c)** 当前证据 RAG 语料小且多为单跳。真引时**只偷侧表思路（`chunk→event/entity`）不整包搬** SAG 的 Fastify + React workbench（与现有 Hono / worker 重叠）；`ai_news_events` 已有 `main_entities` + 事件模型，是**自然嫁接点、非重写**。
