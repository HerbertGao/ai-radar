...
⇒ 修法是 `published_at_authority` **四级**（0 无 / 1 LLM 推断 / 2 程序近似 / 3 页面提取），塌缩与语义合并均改「权威高者胜出 + `GREATEST`」。不变量 `(published_at IS NULL) = (authority = 0)` 使 **NULL-fill 成为该口径的特例**，同权威不覆盖 ⇒ 近似源之间行为零变化。

> **为何 LLM 推断必须【低于】程序近似值**：AI 回填写的是 LLM **猜**的日期，HN 的投稿时刻是**真实时间戳**（只是不是发布日）。若同为一级则「同级不覆盖」⇒ **一个幻觉会永久挡住一个事实** ⇒ 违反第一架构原则（「精确事实由程序和 DB 保障，绝不交 LLM」）。
