...
1. **env 自读文件(~4 行)。** hangar 的 pilot 是**进程内 import**(`executor.ts:71`),env 全靠 daemon 进程共享;实测 ai-radar 与 inbox 撞 `DATABASE_URL`/`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` 三键且**值全不同**——`DATABASE_URL` 撞上是**静默写错库**(它是合法 URL,Zod 挡不住)。解法:`src/config/env.ts:813` 已导出的 `parseEnv(source)` 改喂 `{...process.env, ...自己那份 .env}`,**绝不写 `process.env`**(fire gate 只按 app.id 串行,两个 app 的 run 会并发交错,改全局态即抢)。可行前提 = A0 出口闸「lane 不裸读 process.env」已守住。
