...
- **即时推送复用现有幂等:** 走 `src/push/dispatcher.ts` + `push_records UNIQUE(target_type,target_id,channel,push_date)`。P0 逐条即时推各写一条 `target_type='alert'` 的 push_record;`computePendingSet` 在 **alert 命名空间内**排除 ever-success(同一 P0 不重复即时推)。日报走独立的 `event` 命名空间、看不见 alert 记录——A4 日报会**完整 recap 已 P0 的事件、接受重叠**(不做跨命名空间去重),非零双推。
...
**目标:** 高频扫描 + P0 即时推在位后,把每日 digest 的**新闻段**降级:逐条只产一句话 `headline_zh`(日报唯一显示字段),停产昂贵的 `summary_zh`(~1000 字、日报不显示、仅落库)——不再是"当日精选成稿"。**大体是减法 + 一处搬迁。**

**关键改动:**
- **日报新闻段只产 `headline_zh`:** `src/pipeline/run-daily-workflow.ts` 的 digest 阶段改走 `src/agents/digest/` 的**只产 headline 轻量路径**(Zod schema 仅 `{ headline_zh }`、输出 token 大减),不再逐条产 `summary_zh`。日报消息本就只显示 headline(`src/push/message.ts` 零改)。
- **`summary_zh` 去冗余、收敛到 KB 入库 Agent:** KB 入库(`src/kb/`)本就对每条已推送成功候选逐条跑摘要 Agent(算 `long_term_value` 供准入闸),原与日报 digest **重复**产 `summary_zh`。降级后 `summary_zh` 唯一产出点落在 KB 入库:grounding 原文自产、**原子条件回写** `ai_news_events`(`WHERE summary_zh IS NULL`,只 gate 写回、**绝不** gate Agent 调用),weekly 零 LLM 复用 + KB grounding 不丢,净省日报那次 summary 生成。
- **日报保持完整低频兜底、接受与 P0 内容重叠(不加去重):** 告警(`target_type='alert'`)与日报(`'event'`)是**刻意分开的幂等命名空间**(`src/pipeline/alert-scan.ts` 明载「日报已推同一事件不吞掉告警」);`computePendingSet`/`selectTopN` 只查 `event`、看不见 P0 的 alert push_record,故**日报会完整 recap 已 P0 的事件**。本期不加跨命名空间去重——日报=低频完整兜底、P0=即时高价值。〔修正早稿前提:早稿「`computePendingSet` 已天然排除 ever-pushed → 日报自然不重推 P0、零双推」**是错的**——never-success 排除只在**同一** `target_type` 命名空间内成立,alert 与 event 跨命名空间不去重。〕
- **`product` 中文化 + `experience` mining 两条 LLM 车道本期未动:** 本次只降**新闻段**;product segment(`src/pipeline/product-digest.ts`)、experience mining(`src/pipeline/experience-chain.ts`)的降级/迁移另起(避免 big-bang)。写稿能力(`summarizeEvent` 全路径)不删、只改路由(留供告警链 / 手动全摘要复用)。
- **幂等/锁/时间源全不动:** `push_records UNIQUE(target_type,target_id,channel,push_date)` 4 元组、`daily-digest:{push_date}` 单实例锁 + per-event alert 锁、`computePendingSet` never-success、`getPushDate`(Asia/Shanghai) 时间源、`message.ts` 渲染,**保留不动**。

**DoD:**
- 日报新闻段每条只产/显示 `headline_zh`、不产 `summary_zh`;确定性面(入选 event 排序、`push_records` 行)与降级前一致。
- KB 入库生成 `summary_zh` 写回 `ai_news_events`,weekly 仍零 LLM 复用。
- 降级后 LLM 成本明显下降(日报不再逐条产 ~1000 字 summary)。
- 幂等回归:alert / event 各自命名空间互不吞;日报完整 recap、接受与 P0 重叠。

**出口闸:** 你**不再依赖日报**获取高价值信息(那条路 P0 已接管),日报退化为"扫一眼有没有漏"的低频兜底,你也不觉得可惜。

**显式不做:** 删掉写稿能力(只改路由,RAG 可能复用);动幂等/锁结构;动 `product`/`experience` 两段 LLM 车道。
...
> **闸状态与工作量核查(2026-07-21,查 hangar 仓 + ts.mac-mini 实况):**
> - **闸①机器侧已远超判据**:inbox `483 run/天(*/3)` 连跑 15 天、100% completed、0 failed。出口闸原文「你每天真在用」是人判断,DB 证不了。
> - **闸②已落地**:hangar-view 常驻中。契约极轻——core 只发 `run.started`/`run.completed`,域事件由 app 自定;ai-radar 的 `RunContext` shim 与 hangar `executor.ts:22` 逐字段吻合(其新增的 `triggerKind`/`triggerName` 显式保留 `trigger` 向后兼容)⇒ 不阻塞。
> - **结论:M 不需要 hangar 侧任何改动**,三件事全在本仓侧(见下「M 前置(本仓侧)」)。
> - **排序(用户 2026-07-21 定):与 auto-developer 不排队**——哪个先完成重构哪个先上,ai-radar 不必等 hangar Phase-2。
> - **但 re-host 本身应等 A1 开闸并烤稳**:plan-a 写明「A4 降级前务必确认 A1 的 P0 已被信任」,而实际 A4 已上线、告警车道仍关(2026-07-24 开闸)⇒ 当前「日报已降级 + P0 未开」两头空。下列三件前置无此依赖、可先做。

**M 前置(本仓侧,三件,可在开闸前并行做):**

1. **env 自读文件(~4 行)。** hangar 的 pilot 是**进程内 import**(`executor.ts:71`),env 全靠 daemon 进程共享;实测 ai-radar 与 inbox 撞 `DATABASE_URL`/`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` 三键且**值全不同**——`DATABASE_URL` 撞上是**静默写错库**(它是合法 URL,Zod 挡不住)。解法:已导出的 `parseEnv(source)` 改喂**自己那份 env 文件(唯一来源,不与 `process.env` 叠加)**,**绝不写 `process.env`**——叠加式只护住文件里显式写了的键,schema 里其余 87 个有默认/可选键仍会静默继承兄弟应用的值(详见主规范 `platform-foundation`「环境配置校验」)(fire gate 只按 app.id 串行,两个 app 的 run 会并发交错,改全局态即抢)。可行前提 = A0 出口闸「lane 不裸读 process.env」已守住。

   **provision 时照做(规范里是权威表述,这里是落点——执行的人读的是本文档):**
   - 引导键名 **`AI_RADAR_ENV_FILE`**,值须**绝对路径**;判据是「env 模块首次求值时它在不在 `process.env` 里」,故写进工作目录 `.env` 在 `npm run dev` 下**无效**,compose 的 `env_file:` 注入则**生效**。
   - 该文件须是一份**自足**的 env(7 个必填键 + 托管拓扑下正确的 DB/Redis endpoint);漏写有默认值的键**不报错**,只是静默取 schema 默认(如漏 `FEISHU_*` ⇒ 日报少一条通道且无告警)。
   - **复用现有 `.env` 前须按拓扑核验 endpoint 归属**:compose 给应用的 env 是两层(`env_file` 打底 + `environment:` 覆盖 `DATABASE_URL`/`REDIS_URL`/`PORT`),而 `.env` 里 DB/Redis 那份是 `localhost`。设了引导键会废掉这三键中落在 schema 内的两个。
   - 路径须含本应用标识(如 `/etc/hangar/ai-radar/env`),使「照抄兄弟应用的 provisioning」在值本身即可见。
   - **绝不给 MCP 进程设这个键。** 后果落在同一个 MCP 进程内的两个配置纪元(校验入口读启动环境,推送路径首次调用时才动态 import 主 env 单例),设了会跨目标读取与推送。**该禁令目前只有散文、没有守卫**——见主规范同名残余。
   - 宿主侧须 `await` 本应用的 `import()`,并在 **per-pilot 边界** `try/catch` 住 rejection、记为该 pilot 失败后继续跑其它 pilot;逸出到进程级未处理路径会被 Node 升级为未捕获异常,宿主照样死。
2. **出 `dist/pipeline.js` + `app.yaml`。** hangar DESIGN §3.1:外部 pilot **必须**出编译产物(`.ts` 回退仅限仓内 dev pilot——原生 strip-types 加载不了跨仓带 `.js` specifier 的裸 `.ts`)。本仓现经 tsx 跑源码、无 build。cron 从 BullMQ 搬进 `app.yaml` triggers。
3. **`run()` 自限时(硬要求,非可选)。** hangar `cli.ts` fire gate:「a hung run permanently pins inFlight and that app never schedules again;spine-level watchdog is future scope」,且 executor 的 `ctx.signal` **只在 Ctrl-C/shutdown abort、无时间看门狗**。本仓现只有 per-call 超时 + 锁 TTL,缺整 run deadline ⇒ 一次卡死会让该 lane **永久停排**。

**接受降级(登记,非待修):** pilot 进程内加载 + 跨 app 并发 ⇒ 与 inbox **共享事件循环与崩溃域**。hangar DESIGN §6 与 #18 ADR 均已有意保留((d) 方案 pipeline 留进程内)。
...
- **compose 改写(仅 worker 片):** worker 容器 → hangar daemon + `HANGAR_APPS/{ai-radar,model-radar}` checkout(编出 `dist/pipeline.js`);**web / mcp / browser-worker / RAG / postgres / redis / cloudflared 容器不动**。参考 memory `deployment-containerized`。
  〔**2026-07-21 实况修正:hangar 不是容器**——它在 ts.mac-mini 上是 launchd 原生进程(`hangar-inbox-daemon.sh` + fnm node),`HANGAR_APPS` 下 pilot 为 symlink。故此步要么把 hangar 容器化,要么让本仓 pipeline 出容器裸跑(DB/Redis 仍在 compose、经宿主端口可达)。pilot 自带 `package.json`/`node_modules`,域依赖不进 hangar 仓。〕
