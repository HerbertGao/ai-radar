...
1. **可迁形态(seam-first)。** 所有新/改的流程逻辑写成 `run(ctx)` 形态的**纯 async 函数**,只依赖一个本地 `RunContext` shim(镜像 hangar 的:`input/trigger/config/logger/emit/propose`),**绝不**直接读 BullMQ `job` 对象、进程全局态或调度器。今天由 BullMQ worker 调 `run(makeLocalCtx(...))`,闸后由 hangar 直接调同一个 `run(ctx)`——迁移即换驱动。
...
3. **emit 前向兼容。** `ctx.emit(kind, payload)` 现在落到结构化 pino 日志(可选加一张轻量 `run_events` 表供自查);**事件 kind/payload 形状先粗粒度**,待 hangar monitor 契约落地再对齐。这既给 ai-radar 现在就有 run-chain 可观测性,又是闸后 hangar monitor 直接消费的同一份数据。
...
**关键改动:**
- 新增 `src/pipeline/run-context.ts`:本地 `interface RunContext`(镜像 hangar)+ `makeLocalCtx({ trigger, config, input })`;`emit`→pino(可选 `run_events` 表),`propose`→当前栈无审批,直接执行并 emit `action.executed`(闸后由 hangar gateway 接管)。
- `src/pipeline/run-daily-workflow.ts`(`runDailyWorkflow`)、`src/pipeline/alert-scan.ts`(`runAlertScan`)、`weekly-report.ts` 改签名为 `run(ctx)`;内部用 `ctx.emit` 在**阶段边界**打点(collect/dedup/score/select/push/kb 各一条粗粒度事件),不深埋。
...
- `hangar trace` 式的阶段时间线能从 pino 日志(或 `run_events`)重建。
- 全量测试绿;digest 产出与改造前逐字节/逐记录一致(纯重构无行为变化)。

**出口闸:** 业务逻辑对"谁驱动它"零依赖——grep 不到业务模块 `import` BullMQ / 读 `process.env` 做流程决策(env 只在 ctx.config 装配处读)。
...
**目标:** KB 之上的对话式选型/知识推荐面。**纯读**,DB 为界——服务侧读、pilot 侧写,不写域事实。

**关键改动:**
- **落点(ponytail):** 挂进现有 Hono app(`src/app.ts`),新增 `src/advisor/`(或 `src/rag/`)路由,与 Model Radar web 同进程 SSR/API——自用规模无需独立进程。
- **检索流(SAG 式):** query →(可选 LLM 抽实体)→ BM25/全文命中实体 → 多跳 SQL 沿关系扩展 → rerank 选 top-k → LLM 带引用+检索轨迹作答。复用 `src/agents/llm-client.ts`(结构化输出 + Zod)。
- **未来"按需触发采集"**:先不做;真要,经 `hangar run <collect>` 细缝(闸后),**不**让服务侧写域库(守读写分离)。

**DoD:**
- 就一个真实选型问题(如"预算 X 下的编程订阅怎么选")返回**有 KB 依据、带引用**的回答。
- 服务侧对 Postgres **只读**(代码审计 + 只读连接/权限验证)。
- 命中不到证据时诚实降级("KB 无据"),不编。

**出口闸:** 你日常开始**去问它**(pull 型日常价值成立),回答比翻日报更快解决"选什么/这是什么"。

**显式不做:** 写回 KB;触发采集;多用户/鉴权(自用);把它做成对外产品。
