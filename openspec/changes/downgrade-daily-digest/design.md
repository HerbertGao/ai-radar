...
**D2. `summary_zh` 迁到 KB 入库阶段生成、写回 `ai_news_events`（本质去冗余）。** 现状 KB 摘要 Agent（`generateKbMetadata`）本就在入库阶段对每条已推送候选产出 `{ kb_title, summary_zh, ... }`——与日报 digest 的 `summary_zh` **重复生成**。降级后**日报路径不再产 `summary_zh`**、其产出收敛到 KB 入库（告警链仍为 P0 事件另产 summary+headline，KB 回写 `WHERE IS NULL` 不覆盖）：Agent grounding 于**原文**（`raw_items.content` / 代表标题，不再假设日报预置 `summary_zh`）产出 `summary_zh`，**写回 `ai_news_events.summary_zh`** 供 weekly 零 LLM 复用 + 作 KB grounding。
