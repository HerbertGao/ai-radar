...
> 说明：本节共 **8 条 MODIFIED**——前 7 条为**规范对齐**（只读SSR / age徽标 / 估算 / XSS·CSP / 季年付折算 / 季年付溯源 / 最佳周期）：工作区代码经被吸收的 layout-refresh 已把季/年付明细移入详情行、把「最佳周期」做成主行列、并把状态记号做成 emoji（现状），主规范却仍描述旧 A-3「价格格内子行 + emoji 承载」。前 7 条把这些既有需求同步到「详情行 + 主行最佳周期列 + CSS 记号 + font-src」现实，判定口径 / money-path / effectiveMonthly 取整 / 逐条溯源 / staleness 诚实 / XSS 单闸**一律不改**，仅改「呈现位置」与「状态记号载体（emoji → CSS 绘制 + 文字标签）」+ CSP 加 font-src。第 8 条 **WCAG** 为实质增强（叠加 forced-colors 兜底 / outline 焦点 / accent 定死 / 逐层对比度，且保留原 ①–⑬ 全部 a11y 子条）。避免归档后主规范自相矛盾或丢失细节。
