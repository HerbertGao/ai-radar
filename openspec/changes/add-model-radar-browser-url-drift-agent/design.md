...
- **seed 重播复活漂移前旧 URL（裁决：文档化 + 手动流程）**：url-drift 批准把 `mr_source.source_url` 从 A 改到 B，但 `upsertSource`（`upsert.ts:165`）以 `UNIQUE(vendor_id, source_url)` 为身份键——重跑 seed 时 `seed-data.ts` 仍持旧 URL A，`onConflictDoNothing` 在 `(vendor, A)` 上不撞既有行（该行已迁到 B）→ 以 A **再插一条重复 browser 源**、把死链拉回抓取（继而再触发 staleness/drift → 又开卡）。这是「url-drift 让 `source_url` 可变」与「seed 视 `source_url` 为稳定身份键」的固有张力。**同类**的 `mr_plans.source_url`（并发 plan-edge 插入、或过期 price 批准回写）因抓取只读 `mr_source.source_url`（`scrape-queue.ts` / `fingerprint.ts`，**非** plan 冗余列）而仅是 provenance/展示漂移、不误抓，故降为 minor。**裁决（用户）**：不在本变更改 seed 身份模型（跨子系统），**沿用 `seed-data.ts:22` 既有「再播种 caveat」手动约定**——批准一次 URL 漂移后，运维须同步把 `seed-data.ts` 对应源改成新 URL B（与该文件既有「5d-C 校正了 source_url」的手动流程一致），否则下次重播会复活旧源。**升级路径**：若手动流程被证明易漏（seed 频繁自动重跑），另开提案让 seed 键改稳定身份 或 url-drift 记 old→new 映射供 seed 跟随。
- **大小写变体逃过 `url-conflict` 兜底（DD4 补充）**：DD4 的 `23505` fail-safe 假定既有 sibling 源以 canonical 大小写入库。但 `upsertSource` 存**原样** `source_url`、`mr_source` 唯一键建在**原样** `(vendor_id, source_url)`（`schema.ts:712`）；本变更把**候选**规范化（小写 host）却不动既有源，故候选 `kimi.com/p` 与既有 `KIMI.com/p` 大小写不同 → 不撞 `23505` → 落成两条语义同一源（DD4 兜底失效）。真实修复（ingest 侧规范化 或 `lower(source_url)` 函数唯一索引）跨 ingest+schema 子系统、非本变更范围（同 seed 裁决）。当前语料 host 皆小写、不触发；**残余登记**：接入非 canonical 大小写源前先补 ingest 规范化。
- **hash 路由 SPA 的 fragment 被误剥（D-M7 残余）**：`normalizeUrl` 无条件剥 fragment，假定 `#` 后是纯锚点。hash 路由 SPA（`vendor.com/#!/pricing` vs `#!/enterprise`）里 fragment 是客户端路由、指向不同页——而 browser 档正为 JS SPA 设。当前语料（Z.ai/Kimi）皆 path 路由、不触发。**残余登记**：接入 hash 路由 SPA 源前，把 `normalizeUrl` 改条件剥离（保 `#!`/`#/` 路由段）。
