...
>
> ```
> 阶段 A（commit 1）：cron 判据 + cron 值 + AI 闸 + alertGatePredicate() 共享构造器 + superRefine
...
> ```
>
> **所有可复算 SQL 的 `<threshold>` 一律读生产 `ALERT_IMPORTANCE_THRESHOLD`（`src/config/env.ts:377` 可配），MUST NOT 硬编码 `85`**——生产改阈值 → 判据静默失灵。`<patterns>` = 最终词表逐词包 `%`；**`<negPatterns>` = `NEGATIVE_PATTERNS` 逐词包 `%`（D1.10）——每一条复算 SQL 都 MUST 带那个否定合取项，否则复算的不是真实闸、会把被否定项挡掉的工具帖算进支路 B 的量级**。

## 0. 硬前置门（未全绿 MUST NOT 进入阶段 B「开闸」）

> 阶段 A **不依赖本门**，可先行合并——它独立正确、且与开关无关（车道关着时 AI 闸与共享构造器都是 no-op；车道一开它们是强制项）。

- [ ] 0.1 **`fix-sitemap-published-at` 已归档，且其样板修复落地后重新计时的 7 天生产观察已满。** 在坏基线上观察 7 天 = 假绿信号。
- [ ] 0.2 **`unify-judge-stage` 已归档。** 否则开闸即让日报的正文补全阶段**永久空转**：补全工作集（`src/pipeline/content-enrichment.ts:263-278`）是判分工作集（`src/agents/value-judge/score-events.ts:196`）的**真子集**，两者同键于 `importance_score IS NULL`；告警链每 15–20 分钟判一次分而**不跑补全**，等 08:0x 日报链跑到补全时工作集已空 → `source-content-enrichment` 变死代码。招牌用例是 `hacker_news`（直链帖 `content` 恒 NULL，≈28 事件/天）。
...
- [x] 0.4 **【已跑，2026-07-14 生产只读实测】AI 闸的实测拦截率极低——它是尾部保险，不是高频过滤器。**
      ...
      **闸仍然要加**：fail-closed、与日报要闻闸 `eq(is_ai_related, true)`（`src/selection/top-n.ts:269`）同极性；且生产语料里那条 `should_push=true ∧ is_ai_related=false` 的 `GhostLock` 正说明 Judge 的分类是对的。
...
- [ ] A1.1 写 `expandCronMinutes(pattern: string): Set<number>`：取 cron 第 1 段（分钟字段），按 `,` 切项，每项展开——
      - `*` → `0..59`
      - `*/n` → `{m ∈ [0,59] | m % n === 0}`
      - `a-b` → `a..b`
      - `a-b/n` → 自 `a` 起步进 `n` 且 `<= b`
      ...
      **测试专用工具，不进生产代码路径**（生产不需要在运行时反查 cron 合规）。
- [ ] A1.2 **替换** `env.test.ts:221-225` 的字面量断言：判据改为 `expandCronMinutes(cron) ∩ {0, 30} === ∅`，且**展开抛错即判失败**。
- [ ] A1.3 **覆盖面扩到全部 4 条走飞书发送的定时 cron**（任一违反即失败）：
      | cron | 位置 | 值 | 分钟展开集 |
      |---|---|---|---|
      | `DAILY_DIGEST_CRON` | `src/config/env.ts:323` | `3 8 * * *` | `{3}` ✅ |
      | `ALERT_SCAN_CRON` | `src/config/env.ts:381` | A2 改为 `4-59/15 * * * *` | `{4,19,34,49}` ✅ |
      | **`DEFAULT_WEEKLY_CRON`** | **`src/pipeline/weekly-queue.ts:29`（模块常量，不在 env.ts）** | `7 9 * * 1` | `{7}` ✅ |
      | `MR_PRICE_CURATION_CRON` | `src/config/env.ts:546` | `53 9 * * *` | `{53}` ✅ |
      **`DEFAULT_WEEKLY_CRON` 不是 env 变量**——断言 MUST 直接 import 该常量，**MUST NOT** 因「它不在 env 里」而漏掉它，**MUST NOT** 在测试里抄一份字面量副本（该模块 top-level import `bullmq`；若测试环境不宜引入，把常量提到零依赖叶子模块再由两侧 import）。
      > **不在覆盖面内的 4 条 cron（不发飞书）**：`MR_EVENT_REVIEW_CRON`（`:501`）、**`MR_SCRAPE_HTTP_CRON`（`:507`）与 `MR_SCRAPE_BROWSER_CRON`（`:508`）——`MR_SCRAPE` 是【两条】cron**、`MR_STALENESS_CRON`（`:527`）。
- [ ] A1.4 **展开器单元用例**：`*` / `*/15` / `*/20` / `4-59/15` / `9-59/20` / **`0/15`** / `3` / `a,b,c` 各自的展开集；**非法输入（如 `x` / 空串）MUST 抛错**。
- [ ] A1.5 **负例断言（防判据退回字面量后无人察觉）**：喂 `*/15 * * * *` / `*/20 * * * *` / `* * * * *` / **`0/15 * * * *`** 给判据函数 → MUST 判**违反**。这是本节唯一能证伪「守卫又变回恒绿」的用例。

## A2. cron 值（默认值；生产覆盖值在阶段 B）

- [ ] A2.1 `src/config/env.ts:381`：`ALERT_SCAN_CRON` 的 `.default('*/15 * * * *')` → **`.default('4-59/15 * * * *')`**（展开 `{4,19,34,49}`，仍在规范的「15–30 分钟」保守窗内）。同步更新其上方注释（补一句为何是 `4-59/15` 而非 `*/15`）。
- [ ] A2.2 `.env.example:145-149`：`ALERT_SCAN_CRON=*/15 * * * *` → **`ALERT_SCAN_CRON=4-59/15 * * * *`**，并更新其上方注释块。
- [ ] A2.3 **注释订正**（三处现写「默认 20min」，与 env 默认值 `*/15` 本就不符）：`src/pipeline/worker-main.ts:12`、`src/pipeline/alert-queue.ts:6`、`src/pipeline/alert-queue.ts:66`。

## A3. 告警闸的 AI 闸 + `alertGatePredicate()` 共享构造器

> **一次抽出、两处同调**——这是本变更最重要的结构改动。回填有**固定 LIMIT**（`PUBLISHED_AT_INFERENCE_MAX_PER_RUN` 默认 **20**，`backfill.ts:184-185`）⇒ **回填域比闸宽 = 饥饿**（宽出去的高 importance 非 AI 新闻占掉 20 个名额，把真正在闸内、`published_at IS NULL` 的事件挤出本轮回填 → 永为 NULL → 被 NULL 排除挡住 → **永不告警、可观测无痕迹**）。**「回填域比闸宽只浪费配额」是错的。**

- [ ] A3.1 新建 `src/pipeline/alert-gate.ts`，导出 `alertGatePredicate(threshold: number): SQL`：
      ```ts
...
- [ ] A3.2 `src/pipeline/alert-scan.ts` 的 `selectAlertCandidates`（`:236-258`）：把「`isNotNull(importanceScore)` + `gte(importanceScore, threshold)`」两项**替换为** `alertGatePredicate(threshold)`。其余候选条件（时效窗口 / tombstone / 基线水位 / Model B 去重 / `ALERT_MAX_PER_SCAN`）**一条不减**。
- [ ] A3.3 `src/agents/published-at-inference/backfill.ts` 的 `scopePredicate`（`:103-114`）alert 分支：把「`isNotNull(importanceScore)` + `gte(importanceScore, threshold)`」两项**替换为同一个 `alertGatePredicate(scope.threshold)`**。daily 分支（`eq(shouldPush, true)`）不变。
      ⇒ 回填域 **== 告警闸**（在**每个阶段**都成立），AI 闸一次写对两处。
      **MUST NOT 顺手删掉豁免源谓词**（`fix-sitemap-published-at` 落在此查询上的 `source NOT IN {sitemap}` 合取项）：它是**共享谓词之外的独立合取项**，两条需求不冲突——删了就是把「sitemap 提取失败 → 交 LLM 猜发布日」整条静默改回来。
- [ ] A3.4 **不构成「NULL 永不判分」死锁**（核验，不是改动）：Value Judge 的判分工作集（`src/agents/value-judge/score-events.ts:196`）按 `importance_score IS NULL` 选取、**不带** `is_ai_related` 闸 ⇒ 该列为 NULL 的新事件照常被判分并写入该列。本闸只作用于**判分之后**的候选谓词与回填域。
- [ ] A3.5 **先扩 seed helper**（不扩则 A3.6 / A3.7 的新用例必红）：
...
- [ ] A3.6 **告警闸单测**（`alert-scan.integration.test.ts`，**每条都须钉死 `is_ai_related` / `should_push` 的取值**）：
...
- [ ] A3.7 **回填域测试**（`backfill.integration.test.ts`）：
...
- [ ] A4.1 `src/config/env.ts` 加一条 superRefine（仓内已有先例：`:606` 的 `ALERT_SCAN_ENABLED × ALERT_MIN_PUBLISHED_AT`）：
...
- [ ] A4.2 单测：该组合 → `loadEnv()` 抛错；三者任一不满足 → 正常加载。

## A5. 告警侧判分有界（MUST——一条高频车道必须有界）

> `scoreUnscoredEvents` 的候选 SELECT **既无 `ORDER BY` 也无 `.limit()`**（`src/agents/value-judge/score-events.ts:179-196`；全文 grep `limit|budget|deadline` **零命中**），告警链每轮直接全量送判（`src/pipeline/alert-scan.ts:350`）。
...
- [ ] A5.1 `ScoreEventsOptions` 增 `maxPerRun?: number`（**不新增 env**——守「不新增 env 配置项」非目标）；候选 SELECT **仅在 `maxPerRun` 给出时**加**确定性取序 + `LIMIT`**，**形态唯一、照抄**：
      ```sql
      ORDER BY first_seen_at DESC NULLS LAST, event_id DESC   LIMIT maxPerRun
      --                          ^^^^^^^^^^  ^^^^^^^^^^^^^ 二者【均必需】
      ```
      **🔴 `maxPerRun` 的默认值 MUST 为 `undefined`（＝无 `ORDER BY`、无 `LIMIT`、全量），MUST NOT 是模块常量 N。** 写成 `const limit = options.maxPerRun ?? JUDGE_MAX_PER_SCAN`（4）⇒ **日报链（`run-daily-workflow.ts` 不传该选项）每天只判 4 条** ⇒ ① 要闻段瞬间枯竭；② A5.2 的「不饥饿」论证（老事件由**无界的日报链**在 ≤24h 内排空）**当场坍塌** ⇒ 老事件永久积压。**且全部单测 / 集成测（fixture ≤ 4 条）与两条链的行为逐字相同 ⇒ 恒绿、看不见。** 只有**告警链显式传**。
      **`ORDER BY` MUST NOT 省**（今天既无 ORDER BY 也无 LIMIT）：只加 `LIMIT` ⇒ PG 返回**任意** N 行。**`event_id` MUST NOT 省**：同秒入库的事件 `first_seen_at` 同值时单列排序仍是部分序。
      **🔴 `NULLS LAST` MUST NOT 省**：`aiNewsEvents.firstSeenAt`（`src/db/schema.ts:144`）**无 `.notNull()` ⇒ 可空**，而 PG `ORDER BY x DESC` 默认 **NULLS FIRST**（实测 `(1, NULL, 3)` 上 `DESC LIMIT 1` 返回 **NULL**）⇒ **一行 NULL 即恒排第一、每轮吃掉一个名额、永不老化**（判分工作集无时间剪枝）——与下面「毒事件」是同一个永久饥饿。仓内他处均已写 `DESC NULLS LAST`（`src/pipeline/experience-chain.ts:534`）。
      ...
      **🔴 落 `maxPerRun` 时 MUST 写成 `{ ...options.judge, maxPerRun: N }`——默认值放在 spread 之【后】。** 反过来写（`{ maxPerRun: N, ...options.judge }`）时，任何注入了 `judge` 的测试 / 调用方都会把预算**悄悄抹掉**（`options.judge` 里没有 `maxPerRun` ⇒ spread 覆盖成 `undefined`）⇒ 车道回到无界，而**注入了 judge 桩的集成测恰恰是最可能覆盖到这条路径的那批**。
- [ ] A5.1b **N 的取值 MUST 满足下式（只写「是模块常量」不构成约束）**：
      ```
      N × (F + A×L) < ALERT_SCAN_CRON 周期
      F = COLLECTOR_FETCH_TIMEOUT_MS = 15s（env.ts:349，【env】）
      ...
      L = LLM_TIMEOUT_MS = 60s（env.ts:264，【env】）
      ⇒ 每条最坏 195s ⇒ 15 分钟 cron ⇒ 【N ≤ 4】
      ```
      **反例（MUST 一并读到）**：取 `N=20`（与 `PUBLISHED_AT_INFERENCE_MAX_PER_RUN` 同量级的「自然」选择）⇒ 单轮 `20×195s` = **65 分钟** = cron 周期的 **4 倍** ⇒ `concurrency:1` ⇒ **队列越积越长** ⇒ 车道仍不可用（只是从「死大半天」变成「每轮 1 小时且持续落后」）。**目标没兑现。**
- [ ] A5.1c **🔴 该不等式 MUST 落进 `env.ts` 的 `superRefine`，MUST NOT 只写在散文里**：四项里**三项是 env**（`COLLECTOR_FETCH_TIMEOUT_MS` / `LLM_TIMEOUT_MS` / `ALERT_SCAN_CRON` 周期）⇒ 硬编码一个 `4` + 一段散文 ⇒ **生产上调 `LLM_TIMEOUT_MS`（或把 cron 调密）时约束静默失效** ⇒ 单轮又超周期 ⇒ 积压 ⇒ **P0 车道回到不可用**，无一处变红。
      **同一份提案对 `T > F + A×L + W` 已要求 superRefine 启动期强制**（`unify-judge-stage` 的 spec：「**禁止**只写在文档里」）——**对 N 不能是另一套标准**。
      实现形态择一并显式落地：① N 由 env 派生（`floor(cronPeriodMs / (F + A×L))`，再取 `min(该值, 上限)`）；② N 为模块常量、由 superRefine 校验 `N × (F + A×L) < cron 周期`。
      **错误消息 MUST 报出四项的当前值**（`N` / `F` / `A×L` / cron 周期），否则运维拿到一条不可行动的报错。
      单测：调高 `LLM_TIMEOUT_MS` 使不等式不成立 → `loadEnv()` **抛错**；默认值组合 → 正常加载。
      展开 cron 周期用 A1.1 的 `expandCronMinutes`（同一份文法，MUST NOT 抄第二份）；**周期 = 相邻两次触发的最小间隔**（`{4,19,34,49}` ⇒ 15min）。
- [ ] A5.2 `src/pipeline/alert-scan.ts:350` 的调用传该预算（`{ ...options.judge, maxPerRun: N }`，见 A5.1）；**日报链（`run-daily-workflow.ts`）不传 ⇒ 保持全量**。
...
- [ ] A5.2b **🔴 回归钉：日报链判 > N 条事件（不截断）。** fixture 造 **6** 条未评分事件（> N=4）→ 跑日报链的判分阶段（**不传** `maxPerRun`）→ 断言 **6 条全被判**。
      **这是唯一能证伪「有人把 N 写成缺省值」的用例**——现存全部用例的 fixture 都 ≤ 4 条，两条链行为逐字相同、**恒绿**。
- [ ] A5.3 触顶 MUST 发**结构化可观测事件**（复用既有 run-event 通道，**MUST NOT 新建通道**），**MUST NOT 静默截断**。
...
- [ ] A5.4 **claim / 写 CAS 不变量不变**：被预算挡在本轮外的事件 `importance_score` 仍为 NULL ⇒ **留在工作集里、下一轮继续**（预算只裁单轮工作量，不丢事件、不改「一事件只评一次分」）。`LIMIT N` 天然落在 claim **之前** ⇒ 超预算事件**从未被 claim**、下一轮即刻可取（若落在 claim 之后，它们要等满 `JUDGE_CLAIM_RECLAIM_MS` 才被回收）。
      **不做墙钟 deadline 变体** ⇒ 「MUST 释放飞行中 claim」那条子条款不适用（无飞行中止、无需释放）。
- [ ] A5.5 单测：候选 > 预算时 → 本轮只判 N 条 + 发触顶事件（`budgetExhausted=true`）；**余下的事件下一轮被判**（不丢）；日报链调用不受限（不传预算时行为与今天逐字一致，回归钉见 A5.2b）；**取序确定性**——`first_seen_at` 同值的两条事件按 `event_id DESC` 稳定取；**`first_seen_at IS NULL` 的事件 MUST 排在最后**（`NULLS LAST`）——seed 一条 NULL + N 条非 NULL，断言本轮判的是那 N 条非 NULL 的、NULL 那条**不占名额**（防 PG 的 `NULLS FIRST` 默认让它恒排第一、每轮吃一个名额且永不老化）。

## A6. 阶段 A 验收

- [ ] A6.1 `npm run typecheck` / `lint` / `test` 全绿。
- [ ] A6.2 告警链既有集成测试不因 cron 默认值改动而回归。
- [ ] A6.3 守 `test-no-prod-sends`：本阶段不新增发送路径；确认新增/改动的测试不触真实飞书/Telegram。
...
> **次序不可倒**：`src/config/env.ts:606` 的 superRefine 在 `ALERT_SCAN_ENABLED='true'` 而 `ALERT_MIN_PUBLISHED_AT` 未设置时 **fail-fast 拒启动**——这是**预期保护**（防启用瞬间批量推存量 P0 刷屏），不是需要绕过的 bug。
...
- [ ] B1.4 更新 `DEPLOY.md`「五、实时告警启用」：在现有三步之间插入 B1.2（改 cron 覆盖值）。
...
> **回滚判据必须可复算。** `p0.observed` 经 pino 落进程 **stdout**（`src/pipeline/run-context.ts:68`），仓内**无 run 事件落库、无日志聚合** → 判据若只挂在它上面，在证伪任何东西之前就已不可执行。归因是 DB 列的**纯函数**，故判据一律用 `push_records ⋈ ai_news_events` 复算（无需任何新表/新列/日志基建，可随时重跑）。
...
- [ ] B3.2 **量级判据**——7 天内 alert-success 的 distinct 事件数应落在 **≈ 26 条 / 7 天**（3.67 条/天 × 7；见 0.3）。
      **判据有功效**：日间波动窄（干净日极差 **[3, 4]**，约 **1.3×**，n=3），故 26/7d 是一个能区分信号与噪音的基线，**无需为「统计功效不足」额外留大余量**。**显著且持续**高于 26/7d 才是噪音信号；**单日峰值仍不算**。恒为 0 → 与 0.3 的 `total` 对账。
      ```sql
...
- [ ] B3.3 **命中分数分布（人工抽检）**——逐条看标题，确认没有 KVM CVE / 癌症研究 / 探测器那一类混进来：
      ...
      SELECT e.event_id, e.importance_score, e.is_ai_related, e.should_push,
             e.representative_title, p.channel, p.created_at
      FROM push_records p
      JOIN ai_news_events e ON e.event_id = p.target_id
      WHERE p.target_type = 'alert' AND p.status = 'success'
        AND p.created_at > now() - interval '7 days'
...
- [ ] C1.1 `src/collectors/index.ts` 的 `REALTIME_NEWS_SOURCES`：`['rss','hacker_news','github']` → `['rss','hacker_news','github','sitemap']`。
- [ ] C1.2 同步更新写死「三源」的注释：`src/collectors/index.ts`（模块头三视图说明、`REALTIME_NEWS_SOURCES` 常量注释、`collectSources` 注释）、`src/pipeline/alert-scan.ts`（模块头、采集阶段注释）。注释里说清 sitemap 的成本形状（已见集去重在 per-article fetch 之前 + first-fetch-wins ⇒ 稳态每轮 1 次 `sitemap.xml` GET + 1 次已见集查询）。
- [ ] C1.3 **两条链 MUST 都消费 `perSource` 做源级健康告警**。
      > **实测的当前缺口（2026-07-14 只读核验）——不是「高频链缺一个消费者」，而是【一个消费者都没有】**：
      > - `src/pipeline/alert-scan.ts:334-341`：高频链只读 `collected.items`，**从不读 `collected.perSource`**；
      > - **`src/pipeline/run-daily-workflow.ts`：全文 `perSource` 零命中**（`grep -c perSource` → **0**）；
      > - `classifySystemFailure(stats: CollectStats)`（`src/pipeline/circuit-breaker.ts:92`）的入参**不含 `perSource`**（**观察记述，2026-07-14**：该结构当时只有 `collectedCount` / `newsProcessableCount` 两个字段——**本任务只依赖「它不含 `perSource`」，不依赖它有几个字段**，故 MUST NOT 写成对字段数的现在时断言）；sitemap 单源 throw 被 `allSettled` 隔离 ⇒ 其余源照常返回 ⇒ `collectedCount > 0` ⇒ **`alert: false`**；
      > - `src/pipeline/run-daily-workflow.ts:136` 的 `defaultAlert` 只是 `console.error`，而 `worker.ts` **不传 alert** ⇒ 生产用的就是它。
      > ⇒ **`sitemap.ts:43` 那句「throw → `perSource.ok=false` → 计入告警」在两条链上都是空头支票，一直如此。日报链不会在 24h 后告警——它永远不会告警。** 缺的是**告警出口**（`runRegistry` 确已 `logError` + `console.error`，`src/collectors/index.ts:229-231`），**不是记录本身**。

      - **🔴 先开注入口 —— 告警链【没有】 `AlertSink` 注入口，不补则本条在生产上是空的**：
        - `RunAlertScanOptions`（`src/pipeline/alert-scan.ts:110-130`）的字段为 `channels` / `senders` / `lock` / `digest` / `log` / `publishedAtInfer` / `publishedAtLock` / `emit` / `judge` / `collect` / `threshold` / `windowDays` / `maxPerScan` / `dbh`——**无 `alert`**（日报链的 `RunDailyWorkflowOptions:190` **有**）。⇒ MUST 增 **`alert?: AlertSink`**（复用 `run-daily-workflow.ts:134` 导出的类型；未注入时同样回落 `console.error`）。
        - `createAlertScanWorker({ connection, concurrency })`（`src/pipeline/alert-queue.ts:97`）**无透传口**，`worker-main.ts:160` 亦只传 `connection`。⇒ **`alert-queue` / `worker-main` 这条生产装配路径 MUST 真的把 `createOpsAlertSink(...)` 注进去**；日报链的生产入口同样 MUST 注入（今天用的是 `defaultAlert` = `console.error`）。
      ...
      - **两条链**（`alert-scan.ts` 与 `run-daily-workflow.ts`）都 MUST 读 `collected.perSource`，对 `ok === false` 的**结构性失败**（sitemap 的 `loc_count=0` → 整源 throw 即典型）产出告警——**共用同一个判定与同一个 `dedupKey`**；
      ...
      - **限频 = DB 唯一约束，零新状态**：`createOpsAlertSink` 内部 `INSERT push_records(target_type='ops-alert', target_id=dedupKey, channel, push_date=今日) ON CONFLICT DO NOTHING` ⇒ 命中 0 行即今天已告过 ⇒ 直接 return。⇒ 高频链每天 72–96 轮、sitemap 每轮都 throw ⇒ **第一轮告警，其余 71–95 轮命中唯一键冲突而跳过**；跨进程、跨重启、跨两条链自动去重（**不会双响**）。
      ...
      - **日报链那一份【绝不可省】**：整条车道的回滚路径是 `ALERT_SCAN_ENABLED=false`（B3.5 / C3.1）⇒ **只把源级告警放在高频链，一回滚唯一的告警出口就随之消失**；
      - **MUST NOT 套用日报的「全源返回 0」系统告警**（高频链空轮是常态）。**「不做全源 0 告警」不蕴含「不做源级健康告警」**——两条判据不同；
      - **MUST NOT 只登记后继续吞**。

## C2. 测试（护栏翻转 + 补桩，MUST 与 C1 同 commit）

- [ ] C2.1 **翻转护栏断言** `src/collectors/__tests__/collectors.test.ts`：`REALTIME_NEWS_SOURCES` 对 `sitemap` 由 `not.toContain` → **`toContain`**；同一断言块仍 MUST 断言**不含** `arxiv` / `product_hunt` / `hugging_face_papers` / `blogger` / `show_hn`。`PRODUCT_SOURCES` 仍 `not.toContain('sitemap')`。
- [ ] C2.2 **同步改 `it` / `describe` 描述**——绝不留一个名叫「不含 sitemap」却断言「含 sitemap」的绿用例。
- [ ] C2.3 子集用例（`collectSources：按 source 筛选子集`）的 `perSource` 断言加 `'sitemap'`，**并给该用例的 `collectors` 注入桩补 `sitemap: async () => []`**——否则 `buildRegistry` 回退真实 `collectSitemaps`。
- [ ] C2.4 **补 `src/pipeline/__tests__/alert-scan.integration.test.ts` 的 `collectorsReturning()` 桩**：加 `sitemap: async () => []`。该 helper 被约 20 个用例共用；**不补则这些用例会跑真实 `collectSitemaps()`**——`SITEMAP_SOURCES` 的 env 默认值即 Anthropic，且测试经 `env.ts` 的 `import 'dotenv/config'` 自动加载 `.env` ⇒ ① 真发 HTTP 到 anthropic.com；② 真把文章写进共享测试库；③ 真调 LLM。守「测试绝不触真实外部」红线。
- [ ] C2.5 全量跑 `npm test`——**MUST 逐个确认无网络出站到 anthropic.com**（C2.4 的回归面）。
- [ ] C2.6 **源级健康告警单测**（C1.3）：注入 `perSource = { sitemap: { ok: false, ... } }` 的采集结果 → 断言**两条链各自**都会调 sink、且 `dedupKey === 'source-health:sitemap'`；**同一源当天再次失败**（同进程多轮 / 另一条链）→ 断言 `push_records` 的 `ON CONFLICT DO NOTHING` 命中 0 行 ⇒ **不重发**（限频不靠进程内状态，重启/redeploy 后仍只发一次）；**全源返回 0 条但 `ok: true`**（正常空轮）→ 断言**不告警**（这条是防有人顺手把日报的「全源 0」系统告警搬过来）。**MUST 注入 sender mock**——绝不真发生产通道。
- [ ] C2.7 **🔴「未注入即 stderr」的显式回归断言（C2.6 自己注入 sink ⇒ 它恒绿、证伪不了生产接线）**：
      - **不传** `alert` 调 `runAlertScan()`（`perSource` 含一个 `ok:false` 的源）→ 断言回落写 `console.error`（**不静默**）；
      - **生产装配路径的断言**：`createAlertScanWorker()` 走生产默认分支时 MUST 得到一个**非 `console.error`** 的 sink（即 `createOpsAlertSink` 已被接上）——这是唯一能证伪「字段加了、线没接」的用例。`worker-main` 侧同理。
...
- [ ] C3.2 观察项：
      ...
      - ④ **回填「判不出」的 LLM 重试放大（同一个 72–96×/天 的系数，落在 LLM 调用上——MUST 登记，不得沉默）**：`src/agents/published-at-inference/backfill.ts:230-238` 对判不出只 `undetermined += 1; continue;`，**不写任何标记**（无 attempt 计数、无负缓存，Redis 锁 `finally` 即释放）⇒ 同一条判不出的事件**每 15–20 分钟被重新推断一次**，直到 `first_seen_at` 滑出 3 天窗——最坏 **~288 轮 × 每轮至多 LLM 重试上限次**调用。
...
- [ ] C3.3 **护栏兑现（本阶段修，见 C1.3）+ 措辞订正**：`source-collectors` 的「防静默归零」护栏（sitemap 2xx 但 `loc_count=0` → throw → `perSource.ok=false` → **计入告警**）**在两条链上都无告警消费者**——高频链 `alert-scan.ts:334-341` 不读 `perSource`；**日报链同样不读**（`run-daily-workflow.ts` 零命中；`classifySystemFailure` 的入参不含 `perSource`，`circuit-breaker.ts:92`）。
      ⇒ 站点改版把 sitemap 打成 0 loc 时，两条链都**只落 stderr**（`runRegistry` 的 `logError`，`collectors/index.ts:229-231`）。**这不是「护栏退化为日频」——日报链根本不会在 24h 后告警，它永远不会告警。** 缺的是**出口**，不是记录（**MUST NOT 把缺口往反方向夸大成「连信号都没留下」**）。
      **故 C1.3 修它**（源级健康告警，两条链共用 `dedupKey`、经 ops sink、DB 唯一约束限频），**不再登记为「接受的失效」**。
- [ ] C3.4 **重渲染风暴的量级更正（登记）+ 有界化**：告警链**无熔断阶段**（`alert-scan.ts` 无 `stageShouldAbort`）。LLM 全挂时，单轮 `213 × (3×60s + 15s) = 41,535s` ≈ **11.5 小时**——**不是「只卡一轮 30–60 分钟」**（那个数低估一个数量级）。
      **并发堆积不成立**（`alert-queue.ts:116` 是 `concurrency: 1`，后续 cron 只排队）；**真实风险是单轮长时阻塞 = P0 车道死大半天**。**判分已由 A5 的每轮工作预算封顶（N ≤ 4，见 A5.1b）**；`ALERT_SCAN_ENABLED=false` 是事后止血，**MUST NOT 作为唯一的运行时保护**。
...
- [ ] D1.1 新建 **`src/keywords/precise-fact.ts`**，导出**四**组常量 + 一组共现规则：`PRECISE_FACT_CORE`（取值型，advisor + P0 两侧共享）、`SELECTION_QUERY_EXT`（提问词/主观词，仅 advisor）、`FACT_CHANGE_EXT`（变更词 + 运维泛词，仅 P0）、**`NEGATIVE_PATTERNS`（器物名否定模式，**两个出口共同消费**，见 D1.10）**，外加 `PRECISE_FACT_COOCCUR`（共现，仅 advisor，见 D1.9）。
      **成员以 `specs/conversational-rag/spec.md` 的穷举表为准（那是唯一 SOT，本文件不抄第二份）**；逐词穷举写死，不得留省略号。
      口径：全部小写、多字短语、不含单字、不含 LIKE 元字符（`%` / `_` / `\`）。
...
- [ ] D1.2 **元字符禁令必须可强制**：`precise-fact.ts` **模块加载即断言**（比测试更难忘记）。**遍历集 MUST 含 `NEGATIVE_PATTERNS`**（它同样被渲染成 SQL `LIKE ANY` + TS `includes()` 两个出口 ⇒ 同样会静默分叉）：
      ```ts
      ...
      ```
      `_` 是 LIKE 的**单字符通配符**（实测 `'gptX4 released' LIKE '%gpt_4%'` → `true`），而 TS 侧 `includes()` 把它当**字面量** → **SQL 与 TS 两个出口静默分叉**，没有任何测试会自然发现它。`\` 是 LIKE 默认转义符，同禁。该断言同时兜住 D2.1 的「词表非空」前提。
- [ ] D1.3 **CJK 子串陷阱**：裸 `用量` **MUST NOT 入任何一组**——`'使用量'.includes('用量')` 恒真 → 「ChatGPT 周使用量破 8 亿」会直接触发 P0（一次手机震动、**无 UI 兜底**）。招牌用例「周用量上限提升 50%」由**核心的** `用量上限` 命中（**P0 支路的词源 = `PRECISE_FACT_CORE ∪ FACT_CHANGE_EXT`，不含共现** ⇒ **`用量上限` 一旦离开核心，该招牌用例即在 P0 侧失联**——见 D1.9 末条），删裸 `用量` **零召回损失**。
- [ ] D1.4 **死词自检**（作用域 = **该 gate 的消费集**）：
      - `额度上限` **MUST NOT 加**——`额度` 已在核心内，`'额度上限'.includes('额度')` 恒真。
      - **存量死词也要清**：`性价比最高` ⊂ `性价比`、`哪个划算` / `哪个更划算` ⊂ `划算`、`how much does` ⊂ `how much`（删除**不改变命中集**）。
- [ ] D1.5 **中英必须一致**：`rate limit` 在英文里**正是那个运维词**（「how to handle rate limit errors」/「429」）→ 它与其标准中译 **`速率限制`**、与中文裸 `限流`、与 `usage limit` **MUST NOT 作为裸词进核心**（advisor 侧假阳 = **无兜底拒答**），只进 `FACT_CHANGE_EXT`（仅 P0）。**但「不收裸词」≠「洞只能留着」——见 D1.9 的共现规则。**
      **语义过宽自检**：`报价` / `单价` / `售价` **MUST NOT 进核心**（会命中「H100 售价上调」「OpenAI 报价 X 亿收购」这类市场/并购新闻），只归 `SELECTION_QUERY_EXT`。核心保留 `价格` / `定价` / `pricing`（真·公告措辞）。
- [ ] D1.9 **共现规则 `PRECISE_FACT_COOCCUR`（关掉英文取值型的存量洞，MUST）**：前置闸 MUST NOT 只由裸词 `includes` 构成，MUST 另支持**确定性共现**。**形态 = `NOT(否定项) ∧ 取值意图词 ∧ 事实名词`（否定项一票否决，优先于共现）**，三组词逐词穷举、照抄：
      ```ts
      ...
      **把运维型放回去的是【否定项】，不是共现本身。**
      ...
      - **`用量上限` 恒留核心（处置 MUST NOT 是「移出核心」）**：P0 支路**不消费共现** ⇒ 移出核心 = 招牌用例「周用量上限提升 50%」**静默失联**（它今天由**核心的** `用量上限` 命中，见 D1.3）；且承载它的那条共现规则本身是**死规则**（见上）。**其 advisor 侧运维假阳是已接受代价**（进核验清单，见 D4.3）。
- [ ] D1.10 **🔴 `NEGATIVE_PATTERNS`（器物名否定模式，两个出口共同消费，绝不可省——规范 MUST 了它，而本 tasks 此前【零命中】）**：
      ```ts
...
- [ ] D1.6 **全角变体**：`token 包` 的三个变体 MUST 全部在核心内——半角空格 `token 包` / **全角空格 U+3000 `token　包`** / 无空格 `token包`。半角变体匹配不到全角标题，而中文标题用全角空格分隔中英文是常态；这类漏词**恒不可见**。
- [ ] D1.7 `src/rag/price-gate.ts` 的 `PRICE_GATE_KEYWORDS` 改为消费 `PRECISE_FACT_CORE ∪ SELECTION_QUERY_EXT`（**不再自维护副本**）。`isPriceOrSelectionQuery` 的**签名不变**；**匹配算法由纯 `some(kw => q.includes(kw))` 扩为**：
      ```ts
...
- [ ] D2.1 新建 **`src/keywords/fact-change-gate.ts`**（与词表同目录，依赖 drizzle + schema）。从**同一份词表**（`PRECISE_FACT_CORE ∪ FACT_CHANGE_EXT`）导出**两个出口**：
...
- [ ] D2.1b **🔴 否定合取项 `NEGATIVE_PATTERNS`（D1.10）——两个出口都 MUST 追加，且 SQL 侧 MUST 复用【同一个】 `lower(...)` 表达式**：
      ```ts
...
- [ ] D2.2 **闸判定必须在 SQL 侧**：`src/pipeline/alert-gate.ts` 的 `alertGatePredicate()`（阶段 A 建）把**第 ③ 项换成 OR**：
      ```ts
...
- [ ] D2.3 可观测归因：`p0.observed`（emit 在 `alert-scan.ts:495`）的 `hits[]` 每条**候选**加 `trigger` 与 `matchedKeywords`。
...
- [ ] D2.4 **告警闸单测**（每条都须钉死 `is_ai_related` / `should_push` 的取值）：
...
- [ ] D2.5 **双出口一致性测试**（本阶段最容易埋的雷）：对同一批标题（含大小写混合、含 `null`、含中英混排、含 `token 包` / `token　包` 这类带半角/全角空格的词），断言 `factChangeTitlePredicate()` 的 SQL 命中集 **== `matchFactChangeKeywords()` 的 TS 命中集**。
...
- [ ] D2.6 回填域测试（`alertGatePredicate()` 一改，回填域自动同步）：
...
- [ ] D3.1 **招牌 e2e**：一条 `importance=30` + `should_push=false` + `is_ai_related=true` + 标题命中词表 + 近日 `published_at` 的事件 → 走完「判分 → 回填（跳过，`published_at` 非 NULL）→ 支路 B 闸 → 推送」→ **成功告警**；幂等键 `UNIQUE(target_type='alert', target_id, channel, push_date)` 行为不变。
      **MUST 注入 sender mock、钉死 channels**——绝不真发生产通道。
- [ ] D3.2 **回落路径 e2e（零流量，守的是「不静默丢弃」）**：同上但 `published_at = NULL` 且源为**非豁免源**（如 `rss`；**MUST NOT 用 `sitemap`**——它已被 `fix-sitemap-published-at` 整源豁免出回填域，用它写这条断言即是把豁免写反）→ 断言该事件**落入回填域** → **显式桩掉推断器返回「无法判定」** → `published_at` 保持 NULL → **不告警**。这是**正确的失败方向**：宁可漏，绝不把老文洗成今日发布。
- [ ] D3.2b **豁免优先 e2e**：同 D3.2 但源为 `sitemap` → 断言该事件**不落入回填域**（推断器**零调用**）→ `published_at` 保持 NULL → **不告警**。
- [ ] D3.3 全量测试与类型检查通过。
...
- [ ] D4.1 **🔴 MUST 重跑（词表已变：新增第四组 `NEGATIVE_PATTERNS` + 否定合取项，D1.10/D2.1b）——D4 的常设规则是「改词表【或改采集源集合】后 MUST 重跑」，本变更两者都改了。**
      **上一轮结果（2026-07-13 生产只读实测，24 词、无否定项）三列对照**：① 不带闸（纯词表）= **1** · ② 支路 B 真实命中 = **1** · ③ 高分命中词表 = **0**。
...
- [ ] D4.2 **MUST 随 D4.1 重跑**（上一轮：**已满足**）。支路 B 命中数落在**个位数**量级（实测 **1**），逐条人工确认为真·额度变更。**仅当「不带闸 > 0」而「支路 B 命中 = 0」时**才停下查 grounding（说明 `is_ai_related` 在 title-only grounding 下把支路 B 判空了）——实测 ① = ② = 1，不触发。
      > **重跑时的判据不变**：若改词表后命中数远超量级，或出现 D4.3 的任一类假阳 → **先修词表，不上线**。
- [ ] D4.3 **人工核验清单（逐条看标题，不是只看条数）——MUST 随 D4.1 重新逐条走一遍**（上一轮 2026-07-13：零假阳）：
      | 词 | 在哪一组 | 过宽面 | 假阳落在哪一侧 |
      |---|---|---|---|
...
- [ ] D5.2 **支路 B 的回滚判据 MUST 由 DB 复算得出，MUST NOT 去数 `p0.observed`**（它只进 stdout，仓内无落库无聚合）。
      ...
      **逐条看标题**（判据的另一半——「零非事实变更标题」只能人眼看）：
      ```sql
      SELECT e.event_id, e.importance_score, e.should_push, e.representative_title, p.created_at
      FROM push_records p
      JOIN ai_news_events e ON e.event_id = p.target_id
      WHERE p.target_type = 'alert' AND p.status = 'success'
        AND p.created_at > now() - interval '7 days'
      ...
      WHERE p.target_type = 'alert'
        ...
        AND ( e.importance_score IS NULL
           ...
           OR ( e.importance_score < <threshold>          -- ← 支路 A 不满足
                AND NOT (lower(e.representative_title)    -- ← 且支路 B 也不满足
                         LIKE ANY (ARRAY[ /* <patterns> */ ])
                         AND NOT (lower(e.representative_title)   -- 支路 B 的否定项（D1.10）
                                  LIKE ANY (ARRAY[ /* <negPatterns> */ ]))) ) );
      ```
...
- [ ] 6.1 `openspec-cn validate p0-alert-lane --strict` 通过。
- [ ] 6.2 proposal 的 Modified Capabilities 与 `specs/` 目录**逐一对上**（6 个：`realtime-alerts` / `feishu-push` / `source-collectors` / `model-radar-ingestion` / `conversational-rag` / `published-at-inference`）。
- [ ] 6.3 `npm run typecheck` / `lint` / `test` 全绿。
- [ ] 6.4 守 `test-no-prod-sends`：新增/改动的测试不触真实飞书/Telegram。
