...
0. **告警闸会推非 AI 新闻（设计缺陷，生产实测）。** 支路 A 的闸**只有 `importance >= 阈值`**——**无 `is_ai_related` 闸**。`importance` 衡量「这条新闻有多重要」，**不是「这是不是一条 AI 新闻」**，两根轴正交。生产语料里的 KVM 客户机逃逸 CVE（`importance=95`）、从头合成的活细胞（95）、Linux LUKS 磁盘加密 bug（92）、癌症研究、土卫六探测器、PID 控制器——全是 Hacker News 头版的高 importance 非 AI 新闻。日报**正确地**没推它们（其要闻闸带 `eq(is_ai_related, true)`，`src/selection/top-n.ts:269`）；**关着时无人受害；开闸即把它们推到手机上。**
   ...
   > **AI 闸的实测拦截率极低，如实登记**：在 `is_ai_related` 列**全天生效**的三个干净日（2026-07-11 / 07-12 / 07-13），支路 A 候选 **11** 条中 `is_ai_related IS FALSE` 的为 **0 条**（**0 拦 / 11 过**；07-10 回填日挡掉 1/14）。**它是一道尾部保险，不是一道高频过滤器。**它仍必须加（fail-closed、与日报要闻闸同极性、且生产样本「GhostLock」（标题含该词的一条 `should_push=true ∧ is_ai_related=false` 实例）恰证 Judge 的分类是对的），但本变更 **MUST NOT** 用任何「高频拦截」的数字给它背书。
...
3. **回滚判据当前不可测量。** 「观察 N 天的告警质量」建立在 `p0.observed` 上，而它经 RunContext 的 console.error 结构化 JSON 落进程 **stderr**（`src/pipeline/run-context.ts:56`；仓内未装 pino）——仓内**无 run 事件落库、无日志聚合**。判据在证伪任何东西之前就已不可执行。所幸告警归因是 DB 列的**纯函数**，`push_records ⋈ ai_news_events` 今天就能复算，无需任何新表/新列/日志基建。
...
- **闸与回填域由同一个 `alertGatePredicate()` 共享构造器生成（阶段 A 即抽出，5 行）**：`src/pipeline/alert-scan.ts` 与 `src/agents/published-at-inference/backfill.ts` **同调**。⇒ AI 闸**一次写对两处**、回填域与告警闸**共享同一谓词段**（已评分 ∧ AI ∧（分数 OR 词表）三个合取项；回填另有 NULL / 窗口 / 豁免源 / INNER JOIN 合取项，见 design D3 的「同构仅指共享谓词」），共享段结构上不可能漂移。（回填有固定 `LIMIT PUBLISHED_AT_INFERENCE_MAX_PER_RUN`，默认 20 ⇒ **回填域比闸宽会饿死近期 NULL 事件**——那正是 `published-at-inference` 主规范自己列为要防的危害。「宽出去只浪费配额」不成立。）
...
- **精确事实域词表提为共享常量**（`src/keywords/precise-fact.ts`，零依赖叶子模块）：`/advisor` 前置闸与 P0 支路 B 共享核心 `PRECISE_FACT_CORE` + 各带一组扩展。SOT = `specs/conversational-rag/spec.md` 的穷举表。词表含**第四组 `NEGATIVE_PATTERNS`**（器物名否定模式：`rate limiter`/`限流器`/`速率限制器`）——P0 支路 B 的 SQL 否定合取项与 `/advisor` 的一票否决共同消费（成员与裁决同在 SOT 穷举表）。

- **加两条 superRefine**（不新增 env）：① `ALERT_SCAN_ENABLED='true' ∧ ALERT_FIRST_SEEN_WINDOW_DAYS=0 ∧ ALERT_MIN_PUBLISHED_AT=''` 三元组合使支路 B 作用域扩成**全表历史**（直撞 `policy-push-timeliness`）⇒ 启动即 fail-fast（A4.1）；② 判分预算不等式 `N × (F + A×L + W) < cron 周期`（A5.1c，同以 `ALERT_SCAN_ENABLED='true'` 门控）。**cron 避整点的展开集约束不是 superRefine**——它由测试断言承载、不入运行时（A1）。

- **P0 车道必须有界（基本属性，非可登记风险）**：`scoreUnscoredEvents` 的候选 SELECT **既无 `ORDER BY` 也无 `LIMIT`**（`score-events.ts:242-266`），告警链每轮全量送判 ⇒ 单轮最坏 `213 × (3×60s + 15s) = 41,535s` ≈ **11.5 小时** = **P0 车道死大半天**，而唯一止血是**人翻 `ALERT_SCAN_ENABLED=false`**（事后止血，不是保护）。⇒ 告警侧判分加**每轮工作预算**：候选 SELECT 加 **`ORDER BY first_seen_at DESC NULLS LAST, event_id DESC LIMIT N+1`**（取 N+1 行、只处理前 N 条——+1 仅触顶信号；`NULLS LAST` 与 `event_id` **均必需**——`first_seen_at` 可空且 PG `DESC` 默认 NULLS FIRST 会让 NULL 恒排头、每轮吃名额且永不老化；`event_id` 是同秒入库的确定性 tie-breaker；**只加 `LIMIT` 不加 `ORDER BY` 会让 PG 返回任意 N 行**）。**N MUST 满足 `N × (F + A×L + W) < cron 周期`**（`F=15s` / `A=3` / `L=60s` / `W=JUDGE_WRITE_BUDGET_MS=60s`，与 claim 回收阈值 `T > F+A×L+W` 同一口径 ⇒ 每条最坏 255s ⇒ 15 分钟 cron ⇒ **N ≤ 3**）——取 N=20 则单轮 85 分钟 = 周期的近 6 倍，队列只会越积越长，**车道仍不可用**。**口径区分**：预算口径的每条最坏 = `F + A×L + W`（末次尝试成功仍要写分）；「重渲染风暴 11.5h」的每条 = `F + A×L = 195s`（LLM 全挂 = 全部尝试失败 ⇒ 无写分）——两个口径各自正确，MUST NOT 互相替换。触顶发结构化事件（**由 `alert-scan.ts` emit，`ScoreEventsResult` 加 `budgetExhausted`；MUST NOT 给 `ScoreEventsOptions` 加 `emit`**）、余量下轮续判（claim / 写 CAS 不变量不变）；**日报链保持全量**（它现在承载**两个**理由：一天一次无界是对的 **＋** 它是告警链 `DESC` 取序不饿死老事件的唯一依据）。**并发堆积不成立**——`alert-queue.ts:116` 是 `concurrency: 1`，后续 cron 只排队。

- **高频链补 `perSource` 源级健康告警（日报链已有，勿重复造）**：sitemap 的「2xx 但 `loc_count=0` → throw → `perSource.ok=false` → 计入告警」契约，**日报链今天已消费**——`run-daily-workflow.ts:526` 的 `perSource` 消费循环对 `ok=false` 发 `dedupKey='source-health:<source>'` 告警（`fix-sitemap-published-at` 引入、已归档（git `5c42f19`）；生产经 `run(ctx)` 注入 `buildOpsAlertSink`，**非 `console.error`**）。**缺口只在高频链**：`alert-scan.ts` 不读 `perSource`（已核实零命中）、`RunAlertScanOptions` 无 `alert` 注入口、`alert-queue`/`worker-main` 无透传口（透传口**无需补**——生产注入点在 alert-scan 自己的 `run(ctx)` 包装，不在 worker 工厂）。⇒ **高频链 MUST 补** `perSource` 消费 + `AlertSink` 注入口 + 生产 `buildOpsAlertSink` 接线（经 alert-scan 的 `run(ctx)` 包装，与日报链同型），与日报链**共用同一 `dedupKey='source-health:<source>'`**（自动互相去重、不双响），**限频由推送幂等唯一约束 `UNIQUE(target_type,target_id,channel,push_date)` 承载**（零新状态、跨进程/跨重启/跨链自动去重；**MUST NOT 用 Redis 键或进程内 Map**——后者 redeploy 即复位，「连续 N 轮」永不达标 = 静默不告警）。**回滚安全性不变**：日报链的源级告警出口本就在，回滚 `ALERT_SCAN_ENABLED=false` 只关高频链、不影响它。**不套用日报的「全源 0」系统告警**（高频空轮是常态）——两条判据不同。

- **`/advisor` 前置闸加确定性共现规则**（取值意图词 ∧ 事实名词，如 `(what is|maximum|…) ∧ (rate limit|usage limit)`）：英文取值型提问「what is Claude's rate limit」此前绕过确定性价格闸、落进 KB + LLM 的**非确定性**路径（红线③无确定性保证）。**共现 ∧ 否定项一票否决**——拦取值型、放过运维型（「how to handle **current** rate limit errors」——含意图词 `current`，由否定项 `handle` 放行；**放过运维型靠的是否定项，不是共现本身**——「共现比裸词窄 ⇒ 自动放过运维型」已被实测证伪，见 tasks D1.9），故「不能加裸词」**不蕴含**「这个洞必须留着」。**MUST NOT 用 LLM 复核决定是否拦截**；共现规则**只进 advisor**，不进 P0（P0 的 SQL `LIKE ANY` 形态不变）。

- **回滚 / 观察判据全部落到 DB 复算**：`push_records ⋈ ai_news_events`，SQL **MUST** 带 `p.status='success'`、**MUST** 用 `<threshold>` 参数化（不硬编码 85）、**支路 B 的低分命中 MUST NOT 计入噪音**、**且支路 B 的每一条复算 SQL MUST 带 `NEGATIVE_PATTERNS` 否定合取项**（漏了复算的就不是真实闸）。
...
- `conversational-rag`：「价格/选型确定性前置闸」——前置闸关键词域补齐**取值型**短语；核心只收取值型（假阳无兜底），运维泛词与提问词分归两侧扩展；**新增确定性共现规则 `PRECISE_FACT_COOCCUR`**（取值意图词 ∧ 事实名词，仅 advisor 消费）关掉英文取值型提问的存量洞，**MUST NOT 用 LLM 复核**；词表常量（四组 + 共现）逐词穷举（本能力是唯一 SOT）；LIKE 元字符禁令 + 死词自检 + CJK 子串禁令；**已知过宽词登记补齐 advisor 侧**。
...
- **重渲染风暴（更正数量级 + 有界化）**：站点批量重渲染把 213 篇老文推回窗口时，告警链**无 `stageShouldAbort` 熔断**——LLM 全挂时**单轮**最坏 `213 × (3×60s + 15s) = 41,535s` ≈ **11.5 小时**，不是「30–60 分钟」。**并发堆积不成立**（`alert-queue.ts:116` 是 `concurrency: 1`）；真实风险是**单轮阻塞 = P0 车道死大半天**，故**判分加每轮工作预算**封顶（**N ≤ 3**，见上）。已见集（sitemap 采集器的 DB 去重集：`raw_items` 中 `source='sitemap'` 的 `canonical_url` 全集，权威定义见 source-collectors「增量语义」）在 fetch 之前过滤 ⇒ 风暴只发生**一轮**；`ALERT_SCAN_ENABLED=false` 是事后止血、**不是唯一保护**。
- **「判不出」的 LLM 重试放大（登记，本期不修）**：`backfill.ts:242-243` 对判不出**不写任何标记**（无 attempt 计数、无负缓存）⇒ 同一条事件每 15–20 分钟被重推断一次，直到 `first_seen_at` 滑出 3 天窗——**与「每天 72–96 次 `sitemap.xml` GET」是同一个放大系数**，只是落在 LLM 上。实际域很小（rss/hn/github 实测 0 条 NULL；`sitemap` 已整源豁免出回填域）⇒ 钱不多、本期不强制修；可选补救为 `infer_attempted_at` 列或带 TTL 的负缓存。**MUST 显式登记，不得沉默。**
