...
### Requirement: driver 无关核心 + 薄 run(ctx) 包装
...
#### Scenario: worker 经 run(ctx),测试与 smoke 直调核心
- **WHEN** BullMQ worker 处理器触发某 lane
- **THEN** 它调 `run(ctx)`(包装把 `ctx.emit` 等映进 options 后调核心);而现有 3 个 lane 集成测试与 `smoke.ts` **继续直调 `run*(options)` 核心**注入 mock,不受 `run(ctx)` 包装影响(核心签名仅新增可选 `emit`,故直调方零破坏)

#### Scenario: A0 不引入统一 trigger switch
- **WHEN** 审视 A0 三条 lane
- **THEN** 无一 lane 引入 `switch(ctx.trigger)` 分派(仅 per-function seam)

### Requirement: RunContext 镜像目标脊柱形状且不依赖它
...
#### Scenario: 本地镜像不引脊柱依赖
- **WHEN** grep `run-context.ts`
- **THEN** 无 `import` 任何 `@hangar/*` 包

#### Scenario: trigger 每 lane 有定义值
- **WHEN** 某 lane 经 `run(ctx)` 执行
- **THEN** `ctx.trigger` 持其已定义值(`digest`/`alert-scan`/`weekly-report`),不为 undefined

### Requirement: lane 业务与 BullMQ driver 解耦(直接 import 层面)
...
#### Scenario: 业务模块直接 import 无 driver 符号
- **WHEN** grep 三个 lane **业务模块**(排除 driver 文件)的直接 import
- **THEN** 无 `from 'bullmq'` / `Job` / `job.data` / 经 `./queue.js` 的连接符号(`env` 单例 import 允许)

#### Scenario: 调度行为不变
- **WHEN** A0 落地(含 weekly-queue 拆分、worker-main 重指)
- **THEN** cron 注册(schedule/timezone)行为逐字不变;仅"构造 ctx→调 run(ctx)"的处理器 shim 改动

### Requirement: 按 lane 实有阶段的 run-event 发射与结局
...
#### Scenario: daily 时间线
- **WHEN** 一次完整 daily run 完成
- **THEN** collect→dedup→score→select→digest→push→kb 七阶段事件齐备且有序

#### Scenario: alert 时间线(无 kb)+ 逐 channel 结局
- **WHEN** 一次 alert-scan run 完成
- **THEN** collect→dedup→score→select→push 五阶段事件齐备(无 kb 事件);每个推送 channel 发一条逐项结局事件(无 aborted-degrade)

#### Scenario: 降级/抛错 → run.failed 且 re-throw
- **WHEN** daily 因降级熔断(`WorkflowAbortError`)或 lease-lost / dispatch-failed 抛错
- **THEN** `run(ctx)` 包装 emit 一条 `run.failed` 终态后 **re-throw 原错误**;`run(ctx)` reject,BullMQ job 失败并可按现有重试外壳重试

#### Scenario: daily 正常早退发 run 级终态
- **WHEN** daily run 因锁被占(`skipped-locked`)或无候选(`skipped-no-candidates`)正常早退
- **THEN** emit 一条 run 级终态结局事件(logger spy 可断言其存在)

#### Scenario: weekly 时间线 + 逐 channel 结局
- **WHEN** 一次 weekly run 完成
- **THEN** select→push 两阶段事件齐备;每个推送 channel 发一条逐项结局事件

### Requirement: propose 为前向兼容 shim,A0 定义但不调用
...
#### Scenario: makeLocalCtx propose 单测
- **WHEN** 单测以桩 handler 调 `ctx.propose({tool,args})`
- **THEN** handler 执行、发一条 `action.executed`、结果 resolve(不 park、不审批)

#### Scenario: A0 无调用方
- **WHEN** 审视 A0 各 lane 业务体
- **THEN** 无一处调用 `ctx.propose`
