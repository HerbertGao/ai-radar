...
## 目的
Model Radar（P5 / 5c）读路径：从 `mr_*` 子集构建去规范化只读快照（内容哈希 version/ETag、离散 stale + 待复核聚合、fail-closed），并在其上提供确定性比价/检索 API（model/tool/protocol/currency/budget 过滤、同 (category,currency) 排序、未知价不参与「最便宜」）。桶2 数据红线：已核价才录、未核保持占位、未核 provenance 不冒充已核。常驻周期 rebuild worker + 跨进程失效、Web 比价页（5d）、推荐器（5e）不在本规范。
