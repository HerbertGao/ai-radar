 ...
 * - **Redis 计数、非内存**：`INCR rag:llmcalls:<UTC-date>`——抗进程重启归零、按日期自动滚动、多进程正确。
...
export function dailyCapKey(now: Date = new Date()): string {
  return `rag:llmcalls:${now.toISOString().slice(0, 10)}`;
}
...
export interface DailyCapResult {
  /** true → 未越限、可发起作答；false → 越限或 Redis 不可用，fail-closed 拒绝作答。 */
  allowed: boolean;
 ...
 *
 * Redis 抛错（不可达/超时）→ `{ allowed: false, count: null }`（fail-closed，绝不 fail-open）。
 * EXPIRE 失败不影响本次判定（TTL 是防孤儿键的兜底、非放行前置）——但仍在 try 内，抛错走 fail-closed。
 */
export async function checkAndBumpDailyCap(deps: DailyCapDeps = {}): Promise<DailyCapResult> {
  ...
  const key = dailyCapKey(deps.now ? deps.now() : new Date());
  try {
    ...
    // 成本兜底绝不 fail-open：Redis 不可用即拒绝作答。
    console.error('[rag-daily-cap] Redis 不可用，fail-closed 拒绝作答', err);
