...
// D3.2/D3.2b e2e：跑完整 runAlertScan 链（回填域 + 告警闸同调 alertGatePredicate 的支路 B）。
const { runAlertScan } = await import('../../../pipeline/alert-scan.js');
const { NO_NETWORK_ENRICH } = await import(
  '../../../pipeline/__tests__/enrichment-test-helpers.js'
);
...
async function seedEvent(args: {
  ...
  title?: string;
  /** 代表 raw_item 的 source（默认 SOURCE）。sitemap 用于验证「确定性提取源豁免回填」。 */
  source?: string;
  /** AI 闸取值（默认 true——与 tombstone-visibility / top-n 套件 seed 同约定，保既有用例行为不变）。
   *  alert 作用域经共享构造器 alertGatePredicate 带 `is_ai_related = true`（fail-closed）。 */
  isAiRelated?: boolean | null;
  /** raw_item 的 collapsed 标记（默认 false 保既有用例行为不变）。跑 runAlertScan 全链的用例
   *  MUST 传 true——否则 collapseUncollapsedRawItems 会把该 raw_item 重塌缩成第二个事件，
   *  被 judge 桩评分后污染候选断言（同 alert-scan 套件 seedNullDateScoredEvent 的约定）。 */
  collapsed?: boolean;
}): Promise<string> {
  seq += 1;
  const sid = `${SOURCE}-${Date.now()}-${seq}`;
  const { rows: rawRows } = await pool!.query<{ id: string }>(
    `INSERT INTO raw_items (source, source_item_id, url, canonical_url, title, content, collapsed)
  ...
  const { rows: evtRows } = await pool!.query<{ event_id: string }>(
    // published_at 与 published_at_authority 同写（CHECK）；seed 的非空日期记 1
    // （「非页面确定性提取」档，与 rss/HN/AI 回填同级）。
    `INSERT INTO ai_news_events
       (dedup_key, representative_raw_item_id, representative_title,
        ...
        is_ai_related)
     VALUES ($1, $2, $3, $4, $5, CASE WHEN $5::timestamptz IS NULL THEN 0 ELSE 1 END, $6, $7, $8)
     RETURNING event_id`,
    [
      `dk-${sid}`,
      rawId,
...
describe.skipIf(!databaseUrl)('backfillPublishedAt（真实 DB CAS / 锁 / 剪枝）', () => {
  it('daily 作用域：should_push=true 且 published_at NULL → CAS 回填', async () => {
    ...
    // 回填写 authority=1（「非页面确定性提取」档）：推断值猜的是【文章的发布日】这件对的事，
    // 故与 rss 的 pubDate / HN 的投稿时刻同档、互不覆盖；只有 sitemap 的页面提取值（2）能覆盖它。
    const { rows } = await pool!.query<{ published_at_authority: number }>(
      `SELECT published_at_authority FROM ai_news_events WHERE event_id = $1`,
      [eventId],
    );
    expect(rows[0]!.published_at_authority).toBe(1);
  });

  it('确定性提取源豁免：source=sitemap 的 NULL 事件不进回填域、保持 NULL（绝不回落 LLM）', async () => {
    // sitemap 已有页面确定性提取（extractPublishedAt）、且无任何页面外日期线索 ⇒ 提取失败即保持
    // NULL，绝不转由 LLM 猜（猜错的日期天然落在合理范围内、下游时效闸一道都挡不住 ⇒ 老文当今日突发）。
    const eventId = await seedEvent({
      publishedAt: null,
      firstSeenAt: IN_WINDOW_FIRST_SEEN,
      shouldPush: true,
    ...
    try {
      // 同时 seed 一条非豁免源的 NULL 事件作对照——证明回填【确实在跑】，只是跳过了 sitemap。
      const controlId = await seedEvent({
        publishedAt: null,
        firstSeenAt: IN_WINDOW_FIRST_SEEN,
        shouldPush: true,
        importanceScore: 90,
      ...
      // 对照组被回填（回填在跑）；sitemap 事件保持 NULL（被豁免谓词排除，从未进候选）。
      expect(await fetchPublishedAt(controlId)).not.toBeNull();
      expect(await fetchPublishedAt(eventId)).toBeNull();
      expect(result.attempted).toBeGreaterThanOrEqual(1); // 至少对照组被尝试
    } finally {
      // sitemap 那条 raw_item 的 source ≠ SOURCE，afterAll 的 cleanup 清不到——本用例自清。
      // raw_item 的 canonical_url 带 SOURCE 前缀（seedEvent 里 `https://example.com/${SOURCE}-…`），
      // 据此精确清掉本用例造的那条 sitemap 行，不误删别处的 sitemap 数据。
      await pool!.query(`DELETE FROM ai_news_events WHERE event_id = $1`, [eventId]);
      await pool!.query(
        `DELETE FROM raw_items WHERE source = 'sitemap' AND canonical_url LIKE $1`,
  ...
  it('alert 作用域 AI 闸同构（A3.7）：importance=95 + is_ai_related=false + published_at NULL → 不进回填域', async () => {
    // 回填域与告警闸同调 alertGatePredicate：高分非 AI 事件（KVM CVE 类）永不告警，回填它是纯浪费——
    // 更糟的是它会占掉 LIMIT maxPerRun 的名额，把闸内 NULL 事件挤出本轮（饥饿，design D3）。
    await cleanup();
    const nonAi = await seedEvent({
      publishedAt: null,
      firstSeenAt: IN_WINDOW_FIRST_SEEN,
      shouldPush: false,
      importanceScore: 95,
      isAiRelated: false, // 钉死：高分但非 AI。
    });
    // 对照：闸内（is_ai_related=true）的同分 NULL 事件被正常回填——证明回填在跑、只是排除了闸外事件。
    const aiControl = await seedEvent({
      publishedAt: null,
      firstSeenAt: IN_WINDOW_FIRST_SEEN,
      shouldPush: false,
      importanceScore: 95,
    ...
    // 只有闸内那条进回填域（不为永不告警的事件耗 LLM 配额、不挤占 LIMIT 名额）。
    expect(generateObjectFn).toHaveBeenCalledTimes(1);
    expect(await fetchPublishedAt(nonAi)).toBeNull();
    expect(await fetchPublishedAt(aiControl)).not.toBeNull();
    await cleanup();
  });
...
/**
 * p0-alert-lane D2.6：alert 作用域的回填域随告警闸同步扩支路 B（alertGatePredicate 一处改动、
 * 两个消费点同时生效——回填域 == 告警闸的共享谓词段，结构上杜绝两个方向的漂移，design D3）。
 */
describe.skipIf(!databaseUrl)('alert 作用域支路 B：回填域与告警闸同步（D2.6）', () => {
  it('低 importance(30) + 命中词表 + is_ai_related=true + published_at NULL → 进回填域（被回填）', async () => {
    await cleanup();
    const id = await seedEvent({
      publishedAt: null,
      firstSeenAt: IN_WINDOW_FIRST_SEEN,
      shouldPush: false, // alert 域不看 should_push（支路 B 正是要覆盖 Judge 否决）。
      importanceScore: 30, // < threshold 85：资格只能来自支路 B（标题命中词表）。
      title: 'Claude 调整 API 限流策略', // 命中 FACT_CHANGE_EXT 的「限流」。
      isAiRelated: true,
    });
    const generateObjectFn = inferMock(INFERRED_ISO);
    const result = await backfillPublishedAt({
    ...
    expect(generateObjectFn).toHaveBeenCalledTimes(1);
    expect((await fetchPublishedAt(id))?.toISOString()).toBe('2022-11-30T00:00:00.000Z');
    await cleanup();
  });

  it('低 importance + 未命中词表 + published_at NULL → 不进回填域（既有行为不变）', async () => {
    await cleanup();
    const id = await seedEvent({
      publishedAt: null,
      firstSeenAt: IN_WINDOW_FIRST_SEEN,
      shouldPush: false,
      importanceScore: 30,
      title: 'Historic AI article', // 不含任何词表词——两支路皆不满足，回填它是纯浪费配额。
      isAiRelated: true,
    });
    const generateObjectFn = inferMock(INFERRED_ISO);
    const result = await backfillPublishedAt({
    ...
    expect(result.attempted).toBe(0);
    expect(generateObjectFn).not.toHaveBeenCalled();
    expect(await fetchPublishedAt(id)).toBeNull();
    await cleanup();
  });

  it('低 importance + 命中词表 + is_ai_related=false + published_at NULL → 不进回填域（AI 闸共用）', async () => {
    await cleanup();
    const id = await seedEvent({
      publishedAt: null,
      firstSeenAt: IN_WINDOW_FIRST_SEEN,
      shouldPush: false,
      importanceScore: 30,
      title: 'Introducing our new pricing', // 命中「pricing」但非 AI——闸外事件占 LIMIT 名额 = 饥饿。
      isAiRelated: false,
    });
    const generateObjectFn = inferMock(INFERRED_ISO);
    const result = await backfillPublishedAt({
...
/**
 * p0-alert-lane D3.2 / D3.2b：回落路径与豁免优先 e2e（跑完整 runAlertScan 链）。
 *
 * - D3.2（零流量，守「不静默丢弃」）：命中词表的低分事件 published_at=NULL、源为**非豁免源
 *   （rss，MUST NOT 用 sitemap——它已被 fix-sitemap-published-at 整源豁免出回填域）** → 落入
 *   回填域 → 显式桩掉推断器返回「无法判定」→ NULL 保持 → 不告警。这是正确的失败方向：
 *   宁可漏，绝不把老文洗成今日发布。
 * - D3.2b：同上但源为 sitemap → 不落入回填域（推断器**零调用**）→ NULL 保持 → 不告警。
 *
 * **全部注入 sender mock、钉死 channels、桩掉 LLM——绝不真发 / 绝不触真实外部（test-no-prod-sends）。**
 */
describe.skipIf(!databaseUrl)('D3.2/D3.2b：published_at NULL 的支路 B 事件回落与豁免 e2e', () => {
  /** 计数 sender（绝不真发）。 */
  function mockSender() {
    const s = { calls: 0, async send() { s.calls += 1; } };
    return s;
  }
  /** 四个实时源全空的采集桩（sitemap 必须补桩，否则回退真实 collectSitemaps 触网）。 */
  ...
  /** 全链 e2e 选项：judge 桩即便被调也产出「非 AI + 低分」（永不成为候选），digest 桩抛错降级。 */
  const e2eOpts = (over: Record<string, unknown>) => ({
    now: NOW,
    dbh: db!,
    ...
    maxPerScan: 100,
    threshold: 85,
    collect: { collectors: emptyCollectors },
    judge: {
      judge: {
        generateObjectFn: async () => ({
          object: {
            is_ai_related: false, type: 'news', category: 'x', importance: 10, novelty: 10,
            developer_relevance: 10, hype_risk: 10, should_push: false, reason: 'noop',
          },
        }),
        logError: () => {},
      },
    ...
    digest: {
      generateObjectFn: async () => { throw new Error('digest disabled in test'); },
      maxAttempts: 1,
      logError: () => {},
    },
    ...over,
  ...
  it('D3.2 非豁免源(rss) + NULL published_at → 进回填域 → 推断「无法判定」→ NULL 保持 → 不告警', async () => {
    await cleanup();
    const eventId = await seedEvent({
      publishedAt: null,
      firstSeenAt: IN_WINDOW_FIRST_SEEN,
      shouldPush: false,
      importanceScore: 30, // 低分：告警资格只能来自支路 B。
      title: 'Claude API 速率限制上调', // 命中 FACT_CHANGE_EXT 的「速率限制」（不含器物名）。
      source: 'rss', // 非豁免源（MUST NOT 用 sitemap——用它写这条断言即是把豁免写反）。
      isAiRelated: true,
      collapsed: true, // 防 runAlertScan 的塌缩阶段把该 raw_item 重塌成第二个事件。
    });
    try {
      // 显式桩掉推断器：返回「无法判定」（publishedAt: null）。
      const inferSpy = vi.fn(async () => ({ object: { publishedAt: null } }));
      const sender = mockSender();
      const result = await runAlertScan(
        e2eOpts({
          senders: { telegram: sender },
          publishedAtInfer: { generateObjectFn: inferSpy, maxAttempts: 1, logError: () => {} },
        }) as never,
      );
      // 事件落入回填域：推断器对它被调用（唯一 NULL 候选 → 恰一次）。
      expect(inferSpy).toHaveBeenCalledTimes(1);
      // 无法判定 → NULL 保持（绝不臆造回填）。
      expect(await fetchPublishedAt(eventId)).toBeNull();
      // NULL 被时效闸排除 → 不告警（正确的失败方向：宁可漏，绝不把老文洗成今日发布）。
      expect(result.alertCandidateCount).toBe(0);
      expect(sender.calls).toBe(0);
      const { rows } = await pool!.query(
        `SELECT 1 FROM push_records WHERE target_type = 'alert' AND target_id = $1`,
        [eventId],
      );
      expect(rows).toHaveLength(0);
    } finally {
      // rss 源的 raw_item 不在 afterAll 的 source=SOURCE 清理域内——本用例自清（同 sitemap 豁免用例）。
      await pool!.query(`DELETE FROM ai_news_events WHERE event_id = $1`, [eventId]);
      await pool!.query(
        `DELETE FROM raw_items WHERE source = 'rss' AND canonical_url LIKE $1`,
        [`https://example.com/${SOURCE}-%`],
      );
    }
  });

  it('D3.2b 豁免优先：源为 sitemap → 不落入回填域（推断器零调用）→ NULL 保持 → 不告警', async () => {
    await cleanup();
    const eventId = await seedEvent({
      publishedAt: null,
      firstSeenAt: IN_WINDOW_FIRST_SEEN,
      shouldPush: false,
      importanceScore: 30,
      title: 'Claude API 速率限制上调',
      source: 'sitemap', // 豁免源：发布日只由页面确定性提取产出，提取失败即 NULL、绝不转 LLM 猜。
      isAiRelated: true,
      collapsed: true,
    });
    try {
