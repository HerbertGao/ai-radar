 ...
 *
 * import 全部 9 个 tool 描述符，导出 `allTools` 数组供 server.ts 统一 `registerTool`。
 *
 ...
 *   source_quality / recommend_coding_subscription（handler 内动态 import env-clean build.ts 现 build 快照）/
 *   search_kb（handler 内动态 import env-clean 检索核心 + embed 变体；缺 embedding 凭据 fail-closed，D7）。
...
/** 全部 9 个工具描述符（server.ts 遍历 registerTool）。 */
