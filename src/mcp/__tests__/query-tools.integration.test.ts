...
/**
 * get_today 产品段中文字段（add-product-chinese-digest，design D6，task 8.5）。
 *
 * 产品 DTO：title = name_zh ?? name（中文译名优先、缺回退英文）；tagline = tagline_zh（缺则 null）。
 * 验证已推产品 join ai_products 当前值还原中文（近似语义：join 当前值、非推送快照）；缺中文回退
 * 英文名 + 空简介，结构化形态正确（digestItemSchema 含 tagline 字段）。
 */
describe.skipIf(!canRun)('get_today_ai_digest 产品段中文字段（8.5）', () => {
  /** 造一条带中文列的 product，返回 product_id（DB 生成）。 */
  async function seedProductZh(args: {
    name: string;
    nameZh: string | null;
  ...
  it('已中文化产品：title=name_zh（中文译名）+ tagline=tagline_zh（中文简介）', async () => {
    const pid = await seedProductZh({
      name: `${SRC}EnglishOnly`,
      nameZh: '中文译名甲',
      taglineZh: '一句话中文简介甲。',
      canonicalDomain: `${DOMP}zh-ok.example.com`,
    });
    await seedPush({ targetType: 'product', targetId: pid, channel: 'telegram' });

    const res = (await getTodayTool.handler({}, {})) as CallToolResult;
    const dto = structured<{
      products: Array<{ targetId: string; title: string | null; tagline: string | null; url: string | null }>;
    }>(res);
    const p = dto.products.find((x) => x.targetId === pid)!;
    expect(p).toBeDefined();
    expect(p.title).toBe('中文译名甲'); // 中文译名优先（join 当前值）。
    expect(p.tagline).toBe('一句话中文简介甲。'); // 中文简介。
    expect(p.url).toBe(`https://${DOMP}zh-ok.example.com`);
  });

  it('未中文化产品：title 回退英文 name + tagline=null（结构化形态正确）', async () => {
    const pid = await seedProductZh({
      name: `${SRC}EnglishFallback`,
      nameZh: null,
      taglineZh: null,
      canonicalDomain: `${DOMP}zh-null.example.com`,
    });
    await seedPush({ targetType: 'product', targetId: pid, channel: 'telegram' });

    const res = (await getTodayTool.handler({}, {})) as CallToolResult;
    const dto = structured<{
      products: Array<{ targetId: string; title: string | null; tagline: string | null }>;
    }>(res);
    const p = dto.products.find((x) => x.targetId === pid)!;
    expect(p.title).toBe(`${SRC}EnglishFallback`); // 回退英文名。
    expect(p.tagline).toBeNull(); // 无中文简介 → null。
  });

  it('要闻段 event 的 tagline 恒为 null（产品简介字段不污染 events）', async () => {
    const eventId = await seedEvent({
      dedupKey: `${SRC}ev-tagline-null`,
      representativeTitle: 'EventNoTagline',
      headlineZh: '事件要点',
    });
    await seedPush({ targetType: 'event', targetId: eventId, channel: 'telegram' });
    ...
    const res = (await getTodayTool.handler({}, {})) as CallToolResult;
    const dto = structured<{
      events: Array<{ targetId: string; title: string | null; tagline: string | null }>;
    }>(res);
    const ev = dto.events.find((e) => e.targetId === eventId)!;
    expect(ev).toBeDefined();
    expect(ev.tagline).toBeNull(); // events 段 tagline 恒 null（简介经 title=headlineZh 还原）。
  });
});
