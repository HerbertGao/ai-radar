...
/**
 * 包一层默认注入不触网补全桩。**补全已折进判分入口**，桩经 `judge.enrich` 注入（`ScoreEventsOptions.enrich`）——
 * 本层不再有独立 `enrich` 字段。默认 `judge.enrich = NO_NETWORK_ENRICH`；测试自带 `judge.enrich` 时覆盖默认。
 */
function runDailyWorkflow(options: RunDailyWorkflowOptions = {}) {
  const judge = { enrich: NO_NETWORK_ENRICH, ...(options.judge ?? {}) };
  return runDailyWorkflowRaw({ ...options, judge });
}
...
describe.skipIf(!canRun)('runDailyWorkflow 正文补全阶段（组 G 2.4/5.2/7.1）', () => {
  ...
  it('2.4 补全阶段在判分前富化空 content 代表 raw_item：og:description 原子写回 raw_items.content', async () => {
    ...
    const result = await runDailyWorkflow({
      ...
      dbh: db!,
      collect: { collectors: collectorsReturning(items) },
      // 补全桩经 judge.enrich 注入（补全已折进判分入口）：resolve 公网、fetchImpl 返回带 og:description 的 HTML。
      judge: {
        judge: { generateObjectFn: judgeMock(), maxAttempts: 1 },
    ...
    // 代表 raw_item 的 content 已被 og:description 写回（判分入口内联补全）。
    // 补全命中数不再暴露在 result（已折进 Value Judge 阶段日志/judgeResult）——以 DB 写回为准。
  ...
  it('5.2 补全后正文经 representative_raw_item_id 加载并透传摘要 prompt（非静默空转）', async () => {
    ...
    expect(result.outcome).toBe('pushed');
    // 摘要 prompt 含补全正文（证明判分入口补全后、加载侧已透传 content；否则 forDigest content 恒空 → 静默空转）。
  ...
  it('2.4 补全失败永不拖垮流水线、不进熔断分母（fetch 抛错 → content 保持空 → 退化仅标题照常推）', async () => {
    ...
    const result = await runDailyWorkflow({
      ...
      dbh: db!,
      collect: { collectors: collectorsReturning(items) },
      // 补全 fetch 抛错 → 该条失败、content 保持空、fail-open 照常送判仅标题。
      judge: {
        judge: { generateObjectFn: judgeMock(), maxAttempts: 1 },
        enrich: {
    ...
    // 补全失败不抛断、fail-open；熔断分母只含 judge/digest、不受补全影响。
    expect(result.judge).toEqual({ processed: 1, degraded: 0 });
    expect(result.digest.degraded).toBe(0);
  ...
  it('2.4/7.1 SSRF 守卫拒绝内网/元数据 URL：不写回、fetchImpl 不被调用、计入失败数', async () => {
    ...
    // 被 SSRF 守卫拒绝 → fetchImpl 未被调用、content 未写回（fail-open、照常送判）。
    expect(result.outcome).toBe('pushed');
...
describe('回归护栏：日报链不再独立编排补全（unify-judge-stage 6.5）', () => {
  it('run-daily-workflow.ts 不再 import content-enrichment 的补全入口（防阶段 2.6 被加回）', async () => {
    const { readFileSync } = await import('node:fs');
    const src = readFileSync(new URL('../run-daily-workflow.ts', import.meta.url), 'utf8');
    // 补全已折进判分入口；日报链绝不再 import/调用补全模块（否则「桩还在传、转发被删、测试仍绿」的绊线复发）。
