 ...
 */
function okSummarize(
  out: { name_zh: string; tagline_zh: string; is_ai_related?: boolean } = {
    name_zh: '某中文译名',
    tagline_zh: '某中文简介。',
  },
) {
  // is_ai_related 现为必填字段（design D5）：调用方只给中文列时补默认 true，否则 Zod 校验不过。
  const full = { is_ai_related: true, ...out };
 ...
 *
 * `isAiRelated` 默认 `null`（未判定）：本套件多数用例验证判定步骤前向补判——待判谓词
 * `is_ai_related IS NULL`（design D5 陷阱二），故默认 NULL 使产品进入判定工作集。专测推送候选
 * 闸门 / 已判定幂等跳过的用例显式传 true/false。
 */
async function seedProduct(args: {
  ...
  await pool!.query(
    `INSERT INTO ai_products
       (product_id, name, name_zh, tagline_zh, canonical_domain, last_seen_at, metadata, representative_raw_item_id, is_ai_related)
     VALUES ($1, $2, $3, $4, $5, now(), $6::jsonb, $7, $8)`,
    [
      productId,
...
async function fetchProductZh(productId: string): Promise<{
  ...
  const { rows } = await pool!.query<{
    ...
    `SELECT name, name_zh, tagline_zh, is_ai_related FROM ai_products WHERE product_id = $1`,
    [productId],
  );
  const r = rows[0]!;
  return {
...
describe.skipIf(!canRun)('digestPendingProducts 候选并集 + 中文化（8.2）', () => {
  ...
  it('未判定（is_ai_related IS NULL）的候选产品被判定并中文化：写入 name_zh + tagline_zh + is_ai_related', async () => {
    ...
    expect(mock).toHaveBeenCalledTimes(1); // 仅本套件 1 个待判定产品（per-test 清理保证）。
    const after = await fetchProductZh(pid);
    expect(after.nameZh).toBe('编码助手');
    expect(after.taglineZh).toBe('面向开发者的开源编码助手。');
    // 落库不丢弃（design D5）：is_ai_related 与译名同一次调用产出、写入 ai_products.is_ai_related。
    expect(after.isAiRelated).toBe(true);
  });

  it('已判定（is_ai_related 非 NULL）跳过 LLM（幂等）：不重复调用、不覆盖既有中文', async () => {
    // 幂等谓词现为 is_ai_related IS NULL（design D5 陷阱二）：已判定产品（is_ai_related=true）
    // 不在 pending 集 → 不调 LLM、中文保持原值。
    const pid = await seedProduct({
      suffix: 'already-judged',
      nameZh: '已有译名',
      taglineZh: '已有简介。',
      isAiRelated: true,
    });
    ...
    expect(after.isAiRelated).toBe(true);
    // 本套件该用例无其它待判定产品（per-test 清理）→ mock 完全不被调用。
    expect(mock).not.toHaveBeenCalled();
  });

  it('陷阱二：迁移前已中文化（name_zh 非空、is_ai_related NULL）产品仍被判定步骤选中补判（不死锁）', async () => {
    // 待判谓词以 is_ai_related IS NULL 纳入（**替换 name_zh IS NULL，绝不 AND**）：否则该产品因
    // name_zh 非空被排除、永久 NULL 死锁。name_zh NULL 种子在错误的 AND 实现下也绿、测不到本陷阱，
    // 故此处**必须**用 name_zh 非空 + is_ai_related NULL 的种子。
    const pid = await seedProduct({
      suffix: 'trap2-migrated',
      nameZh: '迁移前译名',
      taglineZh: '迁移前简介。',
      isAiRelated: null,
    });
    // 补判返回不同译名 + is_ai_related=true；COALESCE 须保留既有译名、只补 is_ai_related。
    const mock = okSummarize({
      name_zh: '不应覆盖既有译名',
      tagline_zh: '不应覆盖既有简介。',
      is_ai_related: true,
    ...
    const after = await fetchProductZh(pid);
    // COALESCE 保留既有非空译名（补判不覆盖）。
    expect(after.nameZh).toBe('迁移前译名');
    expect(after.taglineZh).toBe('迁移前简介。');
    // is_ai_related 由 NULL 补判为 true（落库不丢弃）。
    expect(after.isAiRelated).toBe(true);
  });
...
describe.skipIf(!canRun)('selectProductCandidates 读中文列 + 回退映射（8.3）', () => {
  it('中文化产品：representativeTitle=name_zh、headlineZh=tagline_zh', async () => {
    const pid = await seedProduct({
      ...
      nameZh: '中文译名',
      taglineZh: '中文要点简介。',
      isAiRelated: true, // 推送候选默认加 AI 闸门，须 true 才可见。
    });
  ...
  it('未中文化产品：回退英文 name、headlineZh=null（无要点行）', async () => {
    const pid = await seedProduct({
      ...
      nameZh: null,
      taglineZh: null,
      isAiRelated: true, // 推送候选默认加 AI 闸门，须 true 才可见（此处只验证回退映射，非闸门）。
    });
    const candidates = await selectProductCandidates('telegram', db!);
    const c = candidates.find((x) => x.eventId === pid)!;
  ...
  it('选品条件不变：中文化不影响 merge_conflict 排除', async () => {
    const pid = await seedProduct({
      ...
      nameZh: '有中文但冲突',
      taglineZh: '冲突简介。',
      isAiRelated: true, // is_ai_related=true 使排除只可能由 merge_conflict 造成（非闸门）。
...
describe.skipIf(!canRun)('digestPendingProducts 失败语义分层（8.7）', () => {
  it('单产品业务失败（ProductDigestFailureError）：保持 NULL 继续下一个、整步不抛', async () => {
    ...
    const mock = vi.fn().mockImplementation(async (args: { prompt: string }) => {
      ...
      return { object: { name_zh: '成功译名', tagline_zh: '成功简介。', is_ai_related: true } };
    });
    const alert = vi.fn();
    ...
    // 业务失败产品保持 NULL（渲染回退英文 + is_ai_related NULL → 本轮 fail-closed 排除，不拖垮新闻）；
    // 另一产品照常判定并中文化（继续下一个、落 is_ai_related）。
    const fail = await fetchProductZh(pidFail);
    expect(fail.nameZh).toBeNull();
    expect(fail.isAiRelated).toBeNull(); // 判定失败回退 NULL（不误落 false/true）。
    const ok = await fetchProductZh(pidOk);
    expect(ok.nameZh).toBe('成功译名');
    expect(ok.isAiRelated).toBe(true);
  });
  ...
  it('单产品业务失败规模未超阈：整步不抛、不告警（1/2 失败，绝对阈 3、比例阈 0.5）', async () => {
    ...
    const mock = vi.fn().mockImplementation(async (args: { prompt: string }) => {
      ...
      return { object: { name_zh: 'ok', tagline_zh: 'ok。', is_ai_related: true } };
    });
    const alert = vi.fn();
