...
describe.skipIf(!databaseUrl)('正文补全（补抓 + 原子写回 + SSRF 守卫）', () => {
  ...
  it('补抓到全站样板 og:description → 按失败计、content 保持空（不回注、不击穿下游护栏）', async () => {
    // 采集器已把样板 og:description 置 content=null，这些行恰因此落进补全工作集；本阶段重抓【同一张
    // 页面】拿回【同一段样板】。若原样写回 → content 变非空样板 → digest 的 hasContent 护栏不触发 →
    // LLM 拿全站公司简介当正文；且该行永久离开工作集、样板成终身正文。故必须与采集器共享同一个
    // isSiteBoilerplate、写回前判样板（fix-sitemap 的 source-content-enrichment MUST）。
    ...
    expect(result.fail).toBeGreaterThanOrEqual(1); // 按失败计（含命中全站样板数）。
  ...
  it('SSRF：IPv4-mapped IPv6（Node 归一为 ::ffff:a9fe:a9fe 压缩 hex）→ 拒绝、不抓取', async () => {
    // 回归护栏（CodeRabbit critical）：Node WHATWG URL 把 [::ffff:169.254.169.254] 归一为压缩十六进制
    // ::ffff:a9fe:a9fe，仅匹配点分十进制的旧守卫会漏放 → 元数据/内网 SSRF。守卫须解 hex 末 32 位复判 v4。
    const url = 'http://[::ffff:169.254.169.254]/latest/meta-data/';
    const id = await seedCandidate({ content: '', url });
    const { fn, calledUrls } = makeFetchImpl({ [url]: htmlResponse('LEAKED METADATA') });
    ...
    expect(calledUrls).not.toContain(url); // hex 形态也须被守卫拒绝、内网目标绝不被抓取
    expect(await readContent(id)).toBe('');
    expect(result.fail).toBeGreaterThanOrEqual(1);
  });
