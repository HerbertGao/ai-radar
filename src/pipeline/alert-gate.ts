 ...
 * 支路 B（阶段 D）落在本构造器 ⇒ 一处改动、两个消费点同时生效——这正是抽构造器的全部意义。
 *
...
import { factChangeTitlePredicate } from '../keywords/fact-change-gate.js';

/**
 * 告警闸（两项共用前提 + 两条并列确定性支路，design D2）：
 *   importance_score IS NOT NULL
 *   AND is_ai_related = true
 *   AND ( importance_score >= threshold OR representative_title 命中精确事实变更词表 )
 *
 * - ①「已评分」在 OR 之外（评分失败 / claim 被抢 / LLM 降级的 NULL 事件绝不入闸——放进 OR 里，
 *   LLM 当轮全挂时所有含 `pricing` 的采集条目会绕过 Value Judge 直接进推送）。
 ...
 *   两支路各自需要它：支路 A 因 importance 衡量「有多重要」而非「是不是 AI 新闻」（KVM 逃逸 CVE
 *   也能拿 95），支路 B 因无 importance 地板（随机 SaaS 的「Introducing our new pricing」帖）。
 * - ④ 支路 B（factChangeTitlePredicate）：正向 `LIKE ANY` ∧ NOT 器物名否定项，两侧同一个
 *   lower(representative_title)；**恒返回 SQL、永不 undefined**——`and(x, undefined)` 会静默丢项、
 *   OR 塌缩成只剩支路 A（详见 fact-change-gate.ts）。
 * - **MUST NOT 加 `should_push`**——那是 Judge 的「值不值得推」，支路 B 存在的全部意义正是推翻
 *   Judge 对该类事件的低估（唯一生产命中样本 should_push=false，加了它支路 B 恒为空）。
 * - **本期不加 source 谓词**：ai_news_events 的 source 全集当前即 REALTIME_NEWS_SOURCES，源级
 *   谓词筛 0 行；该等式将来被打破时加 source 谓词是正当补救（spec 明文不设 MUST NOT）。
 */
...
export function alertGatePredicate(threshold: number): SQL {
  return and(
    ...
    or(
      ...
      factChangeTitlePredicate(),                             // ④ 支路 B（恒返回 SQL）
    ),
  ) as SQL;
}
