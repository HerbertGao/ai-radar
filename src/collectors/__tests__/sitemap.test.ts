...
// ── 页面确定性发布日提取（extractPublishedAt）+ 全站样板视同缺失（isSiteBoilerplate） ────────

/** 钉死参考时刻毫秒（extractPublishedAt 的 now 上界；用 NOW = 2026-06-14 保持与集成用例一致）。 */
const NOW_MS = NOW.getTime();

describe('extractPublishedAt 纯函数（唯一 h1 + 紧邻元素整串日期）', () => {
  it('① 单 h1 + 紧邻元素文本 `Nov 2, 2023` → Date.UTC(2023,10,2)（ISO 2023-11-02T00:00:00.000Z）', () => {
    const html =
      `<article><h1>Introducing Claude Opus 4</h1>` +
      `<div class="body-3 agate">Nov 2, 2023</div><p>body</p></article>`;
    const d = mod.extractPublishedAt(html, NOW_MS);
    // load-bearing：证明用 Date.UTC 构造、月份缩写映射正确、紧邻元素文本被整串匹配。
  ...
  it('② 两个 h1（站点加了 nav/卡片 h1）→ null（干净失败，不锚第一个去猜）', () => {
    const html =
      `<nav><h1>Site Nav</h1></nav>` +
      `<article><h1>Real Title</h1><div>Nov 2, 2023</div></article>`;
    expect(mod.extractPublishedAt(html, NOW_MS)).toBeNull();
  });

  it('③ 零 h1 → null（干净失败）', () => {
    const html = `<article><h2>No h1 here</h2><div>Nov 2, 2023</div></article>`;
    expect(mod.extractPublishedAt(html, NOW_MS)).toBeNull();
  });

  it('④ 子串陷阱 `Updated Jan 5, 2024`（含合法日期但非整串）→ null（整串全匹配拒绝「找到错的」）', () => {
    // load-bearing 红线：这是「整串全匹配 vs 子串搜索」的证伪器。子串搜索会抠出修订日 Jan 5, 2024，
    // 而该错误日期天然落在合理范围内、下游范围校验/时效窗一道都挡不住 ⇒ 老文被当今日突发。
    const html = `<article><h1>Title</h1><div>Updated Jan 5, 2024</div></article>`;
    expect(mod.extractPublishedAt(html, NOW_MS)).toBeNull();
  });

  it('④b 子串陷阱 `By Jane Doe · Mar 2, 2025 · 5 min read` → null（同上，整串不中）', () => {
    const html =
      `<article><h1>Title</h1><div>By Jane Doe · Mar 2, 2025 · 5 min read</div></article>`;
    expect(mod.extractPublishedAt(html, NOW_MS)).toBeNull();
  });

  it('⑤ 越界：`Jan 1, 2010`（< 2015-01-01 常量下限）→ null', () => {
    const html = `<article><h1>Title</h1><div>Jan 1, 2010</div></article>`;
    expect(mod.extractPublishedAt(html, NOW_MS)).toBeNull();
  });

  it('⑤b 越界：未来日期（> now）→ null', () => {
    // NOW = 2026-06-14；Dec 31, 2026 在未来 → 越界置 null。
    const html = `<article><h1>Title</h1><div>Dec 31, 2026</div></article>`;
    expect(mod.extractPublishedAt(html, NOW_MS)).toBeNull();
  });

  it('⑤c 日历无效 `Feb 30, 2023`（Date.UTC 会静默进位到 3 月）→ null（拒越月回绕）', () => {
    const html = `<article><h1>Title</h1><div>Feb 30, 2023</div></article>`;
    expect(mod.extractPublishedAt(html, NOW_MS)).toBeNull();
  });

  it('⑥ new Date 陷阱：`Dec 31, 2023` 恒解为 UTC 零点（非本地时区），结果 2023-12-31T00:00:00.000Z', () => {
    // 若用 new Date("Dec 31, 2023")，在 UTC+N 时区会解为本地零点 → toISOString 差一天（2023-12-30T…Z）。
    // Date.UTC 构造使结果与运行时时区无关，恒为 UTC 零点。此断言在任意 TZ 下都成立。
    const html = `<article><h1>Title</h1><div>Dec 31, 2023</div></article>`;
    const d = mod.extractPublishedAt(html, NOW_MS);
    expect(d?.toISOString()).toBe('2023-12-31T00:00:00.000Z');
    // 显式核对 UTC 各分量，进一步证明未走本地时区解析。
  ...
  it('⑨ ReDoS 上界：大量 <h1 但无闭合的畸形 HTML → < 100ms（indexOf 线性，非二次方回溯）', () => {
    // 唯一 h1 但其后 ~1MB 无闭合、无 '<' 的文本：naive [\s\S]*?</h1> 会远扫；indexOf 定位线性。
    const oneH1NoClose = `<h1>` + 'a'.repeat(1_100_000);
    expect(oneH1NoClose.length).toBeGreaterThan(1024 * 1024);
    const t0 = Date.now();
    const r1 = mod.extractPublishedAt(oneH1NoClose, NOW_MS);
    // 大量 <h1（含 '>'）无闭合：第二个 h1 立即被计数出 → 直接 null（不进 </h1> 扫描/正则）。
    const manyH1 = '<h1>x'.repeat(250_000);
    expect(manyH1.length).toBeGreaterThan(1024 * 1024);
    const r2 = mod.extractPublishedAt(manyH1, NOW_MS);
    const elapsed = Date.now() - t0;
    expect(elapsed).toBeLessThan(100); // 严格时限；二次方实现会在此卡死。
    expect(r1).toBeNull(); // 唯一 h1 无闭合 </h1> → 干净失败。
    expect(r2).toBeNull(); // >1 h1 → 干净失败。
  });
});

describe('isSiteBoilerplate 纯函数', () => {
  const BOILERPLATE =
    "Anthropic is an AI safety and research company that's working to build reliable, interpretable, and steerable AI systems.";
  it('命中已知 Anthropic 全站样板（trim 后精确匹配）→ true', () => {
    expect(mod.isSiteBoilerplate(BOILERPLATE)).toBe(true);
    expect(mod.isSiteBoilerplate(`  ${BOILERPLATE}  `)).toBe(true); // 前后空白 trim。
  });
  it('真实文章摘要 → false', () => {
    expect(
      mod.isSiteBoilerplate('Claude Opus 4 is our most capable model.'),
    ).toBe(false);
  });
});

describe('collectSitemaps 页面日期提取集成（published_at 由页面确定性提取产出）', () => {
  async function collectOne(html: string) {
    return mod.collectSitemaps({
      sources: [ANTHROPIC_CONFIG],
      fetchText: async () =>
  ...
  it('真实结构文章 → published_at 落值（authority=2 的生产者），metadata.lastmod 仍在', async () => {
    const items = await collectOne(fixture('anthropic-article-dated.html'));
    expect(items).toHaveLength(1);
    const it0 = items[0]!;
    expect(it0.publishedAt?.toISOString()).toBe('2023-11-02T00:00:00.000Z');
    // lastmod 绝不进 published_at，仅入 metadata。
    expect(it0.metadata?.lastmod).toBe('2026-06-13T10:00:00Z');
  });

  it('无紧邻日期的文章（原 anthropic-article.html）→ published_at 仍 null（干净失败）', async () => {
    const items = await collectOne(fixture('anthropic-article.html'));
    expect(items[0]!.publishedAt).toBeNull();
  });
});

describe('collectSitemaps 全站样板 og:description 视同缺失（③ + 顺序）', () => {
  const BOILERPLATE =
    "Anthropic is an AI safety and research company that's working to build reliable, interpretable, and steerable AI systems.";
  // 真实 Anthropic 页面里 og:description 的撇号是 HTML 实体 &#x27;（不是裸 '，裸引号会撞
  // extractOgTag 的属性值引号边界——那是测试 fixture 失真，不是实现 bug）。用实体形态嵌 HTML。
  const BOILERPLATE_HTML = BOILERPLATE.replace(/'/g, "&#x27;");
  function sitemapXml() {
    return `<urlset><url><loc>https://www.anthropic.com/news/claude-opus-4-launch</loc><lastmod>2026-06-13T10:00:00Z</lastmod></url></urlset>`;
  }

  it('⑦ 样板 og:description（有 og:title）→ content=null，但条目照常发射（不跳过）', async () => {
    const html =
      `<!DOCTYPE html><html><head>` +
      `<meta property="og:title" content="Some Real Title" />` +
      `<meta property="og:description" content="${BOILERPLATE_HTML}" />` +
      `</head><body><article><h1>Some Real Title</h1></article></body></html>`;
    const items = await mod.collectSitemaps({
      sources: [ANTHROPIC_CONFIG],
      fetchText: async () => sitemapXml(),
      fetchArticle: async () => html,
      querySeenCanonicalUrls: async () => new Set(),
      now: NOW,
      windowDays: WINDOW_DAYS,
    ...
    expect(items).toHaveLength(1); // 照常发射（非跳过）。
    expect(items[0]!.title).toBe('Some Real Title');
    expect(items[0]!.content).toBeNull(); // 样板视同缺失。
  });

  it('⑧ 顺序：缺 og:title + 样板 og:description → 发 slug-title 条目（content=null），不是整篇跳过', async () => {
    // 反序（先把样板置 null 再判双缺）会把此页从「发 slug-title 条目」翻成「整篇跳过」——未授权行为变化。
    const html =
      `<!DOCTYPE html><html><head>` +
      `<meta property="og:description" content="${BOILERPLATE_HTML}" />` +
      `</head><body><article><h1>Heading</h1></article></body></html>`;
    const items = await mod.collectSitemaps({
      sources: [ANTHROPIC_CONFIG],
      fetchText: async () => sitemapXml(),
      fetchArticle: async () => html,
    ...
    expect(items).toHaveLength(1); // 未跳过：M-1 双缺锚在 og:title，og:description 非 null。
    expect(items[0]!.title).toBe('Claude Opus 4 Launch'); // slug 派生。
    expect(items[0]!.content).toBeNull(); // 样板视同缺失。
  });
});
