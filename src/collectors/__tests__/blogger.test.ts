...
describe('mapBloggerItem：source_item_id fallback 链（绝不为 NULL）', () => {
  ...
  it('对象形态 guid（Atom/YouTube 带属性节点 { _: value }）取 `_`，不抛', () => {
    // 回归：rss-parser 对带属性的 <guid>/<id> 返回对象；直接 .trim() 曾抛 TypeError 拖垮整源（blogger 库内 0 行）。
    const out = bloggerMod.mapBloggerItem(
      { guid: { _: 'g-obj', $: { isPermaLink: 'false' } } as never, title: 'T' },
      BLOG_FEED,
      'v',
    );
    expect(out.sourceItemId).toBe(normMod.sha256Hex(`${BLOG_FEED}\0g-obj`));
  });
});
...
describe('collectBlogger：落库口径 + 字幕增强 + 失败隔离（注入桩不触网）', () => {
  ...
  it('单条目在 mapBloggerItem 内抛错被逐条隔离，其余条目照常发射', async () => {
    // 回归：坏条目曾使整个 collectBlogger 抛出、丢弃全部 feed（blogger 库内 0 行）。
    // 坏条目须真的在 mapBloggerItem 内抛错才覆盖 try/catch —— title 是对象 → (title).trim() 抛。
    const logError = vi.fn();
    const items = await bloggerMod.collectBlogger({
      feeds: [{ url: BLOG_FEED, vendor: 'simonw' }],
      fetchFeed: async () => ({
        items: [
          { title: {} as never, link: 'https://simonwillison.net/bad' },
          { guid: 'ok', title: '好条目', link: 'https://simonwillison.net/ok' },
        ],
      }),
      logError,
      maxAttempts: 1,
