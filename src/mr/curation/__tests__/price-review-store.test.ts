/**
 * 待批记录 store 单测（task 4.3）——**注入 db/tx 桩，无真实 DB / 不触网**（遵守测试守卫）。
 *
 * 覆盖 money-path 分支契约：
 * - 未过期同候选 → no-op（不重复发卡、不 supersede、不 insert）；
 * - 不同候选 → 置旧 superseded + 插新 pending（旧 token 此后 CAS no-op）；
 * - **过期同候选 → supersede + 重发**（防卡死：不因"同候选"而 no-op）；
 * - 过期/未知令牌 claim CAS 0 行 → null（幂等 no-op）；
 * - claim 非空 → 返回行上冻结值。
 *
 * 真实 SQL（now() 有效期谓词、偏唯一索引、FOR UPDATE 并发）由集成测在真实 DB 验（group F/收尾）。
 */
...
const {
  openReviewOrSupersede,
  claimReview,
  markSuperseded,
  markApplyFailed,
} = await import('../price-review-store.js');
...
describe('openReviewOrSupersede', () => {
  it('无既有 pending → opened（插新，不 supersede）', async () => {
    const ops: Op[] = [];
    const db = makeDb(
      { existing: [], inserted: [{ id: 'r-new', token: 'tok-new' }] },
      ops,
    );
    const out = await openReviewOrSupersede(
      { ...baseInput, candidateValue: '45.00' },
      db as never,
    );
  ...
  it('未过期同候选 → no-op（不 supersede、不 insert、不重复发卡）', async () => {
    const ops: Op[] = [];
    const db = makeDb(
      {
        existing: [
          { id: 'r-old', candidateValue: '45.00', currency: 'CNY', expired: false },
        ],
  ...
  it('不同候选 → 置旧 superseded + 插新 pending', async () => {
    ...
    const out = await openReviewOrSupersede(
      { ...baseInput, candidateValue: '48.00' },
      db as never,
    );
    expect(out).toEqual({
  ...
  it('过期同候选 → supersede + 重发（防卡死，不因同候选 no-op）', async () => {
    ...
    const db = makeDb(
      {
        existing: [
          { id: 'r-old', candidateValue: '45.00', currency: 'CNY', expired: true },
        ],
        inserted: [{ id: 'r-new', token: 'tok-new' }],
      },
      ops,
  ...
  it('非法币种被 Zod 闸在发 SQL 前拒（不进事务）', async () => {
    const ops: Op[] = [];
    const db = makeDb({ existing: [] }, ops);
    await expect(
      openReviewOrSupersede(
        { ...baseInput, currency: 'GBP', candidateValue: '45.00' },
        db as never,
      ),
    ).rejects.toThrow();
...
describe('claimReview CAS', () => {
  it('过期/未知令牌 → CAS 0 行 → null（幂等 no-op）', async () => {
    const ops: Op[] = [];
    const db = makeDb({ updateReturning: [] }, ops);
    const out = await claimReview('dead-token', 'approver-1', db as never);
    expect(out).toBeNull();
  });

  it('命中 → 置 approved 并返回行上冻结值', async () => {
    const ops: Op[] = [];
    const frozen = {
      id: 'r-1',
      planId: 'plan-1',
    ...
    const out = await claimReview('tok-live', 'approver-1', db as never);
...
describe('markSuperseded / markApplyFailed（键 id AND status=pending）', () => {
  it('markSuperseded 命中 1 行 → 1（写 decided_by 审计）', async () => {
    const ops: Op[] = [];
    const db = makeDb({ updateReturning: [{ id: 'r-1' }] }, ops);
    expect(await markSuperseded('r-1', 'approver-1', db as never)).toBe(1);
    const upd = ops.find((o) => o.op === 'update');
    expect(upd?.set?.status).toBe('superseded');
    expect(upd?.set?.decidedBy).toBe('approver-1');
  });

  it('markApplyFailed 0 行（已被并发 supersede）→ 0，不误标（写 decided_by 审计）', async () => {
    const ops: Op[] = [];
    const db = makeDb({ updateReturning: [] }, ops);
    expect(await markApplyFailed('r-1', 'approver-1', db as never)).toBe(0);
    const upd = ops.find((o) => o.op === 'update');
