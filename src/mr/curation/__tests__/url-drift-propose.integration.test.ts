...
describeIfDb('runUrlDriftCuration 真 SQL（propose → review 行 + metric 行）', () => {
  ...
  it('卡片文本过长（>MAX_MESSAGE_LENGTH）→ 跳过、不发卡、不开 review 行（卡装配长度守卫）', async () => {
    await seed('toolong');
    const runId = `${PREFIX}run-toolong`;
    // CJK 病态候选：normalizeUrl 百分号编码把 raw 517 → normalized 4517，卡片显示文本即远超 Telegram 限（任一计数口径下都超）。
    // longDetect 对**每条** pending flag 都返回此超长候选 → 全部命中长度守卫 → 无一 carded、本 run 不开任何 review 行。
    const longCandidate = 'https://kimi.com/' + '中'.repeat(500);
    const longDetect = vi.fn(async () => ({
      kind: 'candidate' as const,
      candidate_url: longCandidate,
      confidence: 'high' as const,
      reason: '会员页迁移',
    }));
    const spyNotify = { telegram: vi.fn(async () => {}) };

    try {
      const res = await runUrlDriftCuration({ notify: spyNotify, runId, dbh: db!, detectFn: longDetect });

      // 无一成卡（守卫在发卡前拦下）——若无守卫，kimi 源会 carded。skipped≥1（本 run 处理全库 pending flag，不钉精确值）。
      expect(res.carded).toBe(0);
      expect(res.skipped).toBeGreaterThanOrEqual(1);
      expect(spyNotify.telegram).not.toHaveBeenCalled();
      const reviews = await db!
        .select({ id: schema.mrUrlDriftReview.id })
        .from(schema.mrUrlDriftReview)
        .where(eq(schema.mrUrlDriftReview.runId, runId));
      expect(reviews).toHaveLength(0);
    } finally {
      // 本用例跳过、未开 review，残留的 pending flag + source 会被后续用例当作新 kimi 源再处理（污染 total_candidates）——须清。
      await db!.delete(schema.mrUrlDriftMetric).where(eq(schema.mrUrlDriftMetric.runId, runId));
      await db!.delete(schema.mrReviewFlag).where(eq(schema.mrReviewFlag.targetId, `${PREFIX}toolong`));
      await db!.delete(schema.mrSource).where(eq(schema.mrSource.id, `${PREFIX}toolong`));
    }
  });
