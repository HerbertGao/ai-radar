...
describe('7.2 支撑：cheapestInfo（已核 plans.length≥2 才出、跨引 unknownCount）', () => {
  ...
  it('停售 plan 可列出但不计入最划算可比候选', () => {
    const discontinued = known('A', '1', 'CNY', { availability: 'discontinued' });
    const live = known('B', '49', 'CNY', { availability: 'on_sale' });
    const info = cheapestInfo(group({ plans: [discontinued, live], cheapestPlanId: 'B' }), 0);
    expect(info.showCheapest).toBe(false);
    expect(info.cheapestPlanId).toBeNull();
  });
...
describe('7.1 支撑：freshnessSortKey / sortPlansByFreshness（null 源最陈旧）', () => {
  ...
  it('periodPrices 的 provenance date 参与最旧 fact 判定', () => {
    const p = known('A', '30', 'CNY', {
      provenance: { sourceUrl: 'https://x', sourceConfidence: 'official_pricing', lastCheckedDate: '2026-06-20' },
      periodPrices: [
        {
          ...
          currency: 'CNY',
          priceStatus: 'known',
          provenance: prov({ lastCheckedDate: '2000-01-01' }),
          effectiveMonthly: 10,
        },
      ],
    });
    expect(freshnessSortKey(p)).toBe('2000-01-01');
    expect(oldestFactBadge(p, new Date('2026-06-29T00:00:00Z')).days).toBeGreaterThan(9_000);
  });
