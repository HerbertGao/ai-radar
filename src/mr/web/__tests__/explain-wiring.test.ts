...
describe('4.4 web llm 路径：模板段 + 叙述段 + 参考清单，且装配确实发生', () => {
  it('注入 explainer（真 buildExplainer + mock LLM + mock 装配）⇒ 页 explanation 含叙述段与参考清单标题', async () => {
    ...
    // 组 D 接线后 llm 路径会读缓存：注入 getCached=miss 桩使本用例保持 hermetic（不触真 Redis）。此处注入的
    // explainer 未接 onRender/beforeLlmCall ⇒ renderedBy 恒 undefined ⇒ 不写缓存、不触 cap，只验「装配→叙述」通道。
